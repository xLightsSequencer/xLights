...
// Geometric point ordering over the parent model's node cloud — the
// desktop's "Sort Strand Points Geometrically". Reorders the nodes within
// display rows [firstRow, lastRow] of `ranges`; pass 0 and count-1 to do
// the whole submodel. `choice` is one of `submodelOrderPointsChoices`.
// Returns the new strand list, or nil on an unrecognised choice or a model
// whose nodes have no single screen position. Like symmetrizeRanges this
// needs the bridge's 2D preview for node positions.
- (nullable NSArray<NSString*>*)orderPointsInRanges:(NSArray<NSString*>*)ranges
                                        ...
                                        forDocument:(XLSequenceDocument*)doc
    NS_SWIFT_NAME(orderPointsInRanges(_:onModel:choice:firstRow:lastRow:forDocument:));

// The ordering modes `orderPointsInRanges` accepts, in the order the
// desktop menu lists them. Generated by shared core so the two platforms
// cannot drift.
