...
typedef enum { VM_NORMAL,
               VM_90,
               VM_270,
               VM_FLIP_HORIZONTAL,
               VM_FLIP_VERTICAL } VMROTATION;
typedef enum { RGB,
               RGBW } VMPIXELCHANNELS;
...
class VirtualMatrix {
    ...
    wxXmlNode* _modelNode = nullptr;
    std::vector<std::vector<std::vector<int>>> _customModelData;
    long _strings = 0;
    long _nodes = 0;
    long _strandsPerString = 1;
    MMORIENTATION _orientation = MMORIENTATION::HORIZONTAL;
    MMSTARTLOCATION _startLocation = MMSTARTLOCATION::TOP_LEFT;
...
public:
    ...
    VirtualMatrix(OutputManager* outputManager, int width, int height, bool topMost, const std::string& rotation, const std::string& pixelChannels, const std::string& quality, const std::string& startChannel, const std::string& name, wxSize size, wxPoint loc, bool useMatrixSize, int matrixMultiplier, const std::string& fromModel);
    ...
    size_t GetChannels() const {
        return _width * _height * GetPixelChannelsCount();
    }
    void SetStartChannel(const std::string& startChannel) {
        if (startChannel != _startChannel) {
    ...
    bool GetUseMatrixSize() const {
        return _useMatrixSize;
    }
    int GetMatrixMultiplier() const {
        return _matrixMultiplier;
    }
    std::string GetRotation() const {
        return DecodeRotation(_rotation);
    }
    std::string GetPixelChannels() const {
        return DecodePixelChannels(_pixelChannels);
    }
    std::string GetScalingQuality() const {
        return DecodeScalingQuality(_quality, _swsQuality);
    }
    bool IsDirty() const {
    ...
    void SetUseMatrixSize(const bool useMatrixSize) {
        if (useMatrixSize != _useMatrixSize) {
            _useMatrixSize = useMatrixSize;
            _changeCount++;
        }
    }
    void SetMatrixMultiplier(const int matrixMultiplier) {
        if (matrixMultiplier != _matrixMultiplier) {
            _matrixMultiplier = matrixMultiplier;
            _changeCount++;
        }
    }
    void SetScalingQuality(const wxImageResizeQuality quality, int swsQuality) {
        if (quality != _quality || swsQuality != _swsQuality) {
            _quality = quality;
            _swsQuality = swsQuality;
            _changeCount++;
        }
    }
    void SetRotation(const VMROTATION rotation) {
        if (rotation != _rotation) {
            _rotation = rotation;
            _changeCount++;
        }
    }
    void SetLocation(const wxPoint location) {
