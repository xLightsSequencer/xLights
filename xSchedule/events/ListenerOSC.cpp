...
void ListenerOSC::Start() {
    ...
    LOG_DEBUG("OSC listener starting.");
...
void ListenerOSC::Stop() {
    ...
    if (!_stop) {
        LOG_DEBUG("OSC listener stopping.");
        if (_socket != nullptr)
...
void ListenerOSC::StartProcess(const std::string& localIP) {
    ...
    if (_socket == nullptr) {
        LOG_ERROR("Error opening datagram for OSC reception on port %d. %s", port, (const char*)localaddr.IPAddress().c_str());
    } else if (!_socket->IsOk()) {
        LOG_ERROR("Error opening datagram for OSC reception on port %d. %s OK : FALSE", port, (const char*)localaddr.IPAddress().c_str());
        delete _socket;
        _socket = nullptr;
    } else if (_socket->Error()) {
        LOG_ERROR("Error opening datagram for OSC reception on port %d. %d : %s %s", port, (int)_socket->LastError(), (const char*)DecodeIPError(_socket->LastError()).c_str(), (const char*)localaddr.IPAddress().c_str());
        delete _socket;
        _socket = nullptr;
    } else {
        _socket->SetTimeout(1);
        _socket->Notify(false);
        LOG_INFO("OSC reception datagram opened successfully on port %d.", port);
        _isOk = true;
    }
}
...
void ListenerOSC::Poll() {
    ...
    if (_socket != nullptr) {
        ...
        // wxStopWatch sw;
        // LOG_DEBUG("Trying to read OSC packet.");
