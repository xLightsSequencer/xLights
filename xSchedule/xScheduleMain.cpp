...
xScheduleFrame::xScheduleFrame(wxWindow* parent, const std::string& showdir, const std::string& playlist, wxWindowID id)
{
    ...
    if (xlconfig != nullptr)     {
        ...
        if (userEmail != "noone@nowhere.xlights.org" && userEmail != "") LOG_DEBUG("User email address: <email>%s</email>", (const char*)userEmail.c_str());
        delete xlconfig;
        xlconfig = nullptr;
    }
    ...
    LOG_DEBUG("xSchedule UI %d,%d %dx%d.", x, y, w, h);
    ...
    LOG_DEBUG("Loading schedule.");
    LoadSchedule();

    if (__schedule == nullptr)     {
        LOG_ERROR("Error loading schedule. Closing xSchedule.");
        Close();
    }
...
#ifndef NDEBUG
    LOG_DEBUG("xSchedule Crash Menu item not removed.");
#ifdef _MSC_VER
    if (IsDarkMode()) {
...
void xScheduleFrame::LoadSchedule()
{
    ...
    LOG_DEBUG("Adding IPs to ping.");
    AddIPs();

    LOG_DEBUG("Update the playlist tree.");
    UpdateTree();

    LOG_DEBUG("Creating buttons.");
    CreateButtons();

    LOG_DEBUG("Schedule loaded.");
}
...
{
    LOG_DEBUG("xScheduleFrame destructor start.");
    ...
    LOG_DEBUG("xScheduleFrame destructor end.");
}

void xScheduleFrame::OnQuit(wxCommandEvent& event)
{
    LOG_DEBUG("OnQuit called.");
...
void xScheduleFrame::SaveShowDir() const
{
    ...
    LOG_DEBUG("Saving show folder location %s.", (const char *)sd.c_str());
    if (!config->Write(_("SchedulerLastDir"), sd))
    {
        LOG_ERROR("Error saving 'SchedulerLastDir'.");
    }
    config->Flush();
}
...
{
    wxConfigBase* config = wxConfigBase::Get();
    LOG_DEBUG("Config: AppName '%s' Path '%s' Entries %d Groups %d Style %ld Vendor '%s'.", (const char *)config->GetAppName().c_str(), (const char *)config->GetPath().c_str(), (int)config->GetNumberOfEntries(), (int)config->GetNumberOfGroups(), config->GetStyle(), (const char*)config->GetVendorName().c_str());

    // get the show directory
    wxString showDir = ScheduleManager::xScheduleShowDir();
    if (showDir == "")
    {
        LOG_DEBUG("Could not read show folder from 'SchedulerLastDir'.");
        showDir = ScheduleManager::xLightsShowDir();
        if (showDir == "")
        {
            LOG_DEBUG("Could not read show folder from 'xLights::LastDir'.");
            DirDialog1->SetPath(_showDir);

            if (DirDialog1->ShowModal() == wxID_OK)
            {
                _showDir = DirDialog1->GetPath().ToStdString();
                LOG_DEBUG("User selected show folder '%s'.", (const char *)_showDir.c_str());
                SaveShowDir();
            }
            else
            {
        ...
        {
            LOG_DEBUG("Read show folder from 'xLights::LastDir' location %s.", (const char *)showDir.c_str());
    ...
    {
        LOG_DEBUG("Read show folder from 'SchedulerLastDir' location %s.", (const char *)showDir.c_str());
        _showDir = showDir.ToStdString();
    }
}
...
void xScheduleFrame::On_timerTrigger(wxTimerEvent& event)
{
    ...
    logger->info("Timer: Start frame elapsed {} now {} last {}", elapsed, now, lastms);
    ...
    if (elapsed < _timer.GetInterval() / 2)
    {
        ...
        if (shortFramesSkipped % 200 == 1) {
            logger->warn("Timer: Frame event fire interval {}ms less than 1/2 frame time {}ms. {} events skipped.", elapsed, _timer.GetInterval() / 2, shortFramesSkipped);
        }
        else {
            logger->debug("Timer: Frame event fire interval {}ms less than 1/2 frame time {}ms. {} events skipped.", elapsed, _timer.GetInterval() / 2, shortFramesSkipped);
        }
        return;
    }
    ...
    if (elapsed > _timer.GetInterval() * 4)
    {
        if (lastms != 0 && __schedule->IsOutputToLights())
        {
            ...
            // log slow and fast frames infrequently
            if (longFrames % 200 == 1) {
                logger->warn("Timer: Frame interval greater than 200%% of what it should have been [{}] {} : So far {}", _timer.GetInterval() * 2, (int)(now - lastms), longFrames);
            }
            else {
                logger->debug("Timer: Frame interval greater than 200%% of what it should have been [{}] {} : So far {}", _timer.GetInterval() * 2, (int)(now - lastms), longFrames);
            }
    ...
    if (ms > _timer.GetInterval())
    {
        ...
        logger->debug("Timer: Frame took too long {} > {} so next frame forced to be output", ms, _timer.GetInterval());
    ...
    logger->info("Timer: End Frame: Time {}", ms);
}
...
void xScheduleFrame::UpdateSchedule()
{
    ...
    auto logger = spdlog::get("frame");
    wxStopWatch sw;
    logger->debug("Updating the schedule.");

    TreeCtrl_PlayListsSchedules->Freeze();

    int rate = __schedule->CheckSchedule();

    logger->debug("Schedule checked {}ms", sw.Time());
    ...
    logger->debug("    Tree updated {}ms", sw.Time());
    ...
    logger->debug("    Timers sorted {}ms", sw.Time());

    UpdateUI();

    logger->debug("    UI updated {}ms", sw.Time());

    TreeCtrl_PlayListsSchedules->Thaw();
    TreeCtrl_PlayListsSchedules->Refresh();

    logger->debug("    Schedule updated {}ms", sw.Time());
}
...
void xScheduleFrame::ChangeShowFolder(wxCommandEvent& event)
{
    ...
    LOG_DEBUG("Changing show folder to %s.", (const char*)newShowFolder.c_str());
...
void xScheduleFrame::OnMenuItem_ViewLogSelected(wxCommandEvent& event)
{
    ...
    if (fn != "" && ft) {
        ...
        LOG_WARN("Unable to view log file %s.", (const char*)fn.c_str());
...
void xScheduleFrame::UpdateStatus(bool force)
{
    ...
    logger->debug("            Status Text {}ms", sw.Time());
    ...
    logger->debug("            Got selected playlist {}ms", sw.Time());
    ...
    logger->debug("            Updated running listview {}ms", sw.Time());
    ...
    logger->debug("            Updated toolbar {}ms", sw.Time());
    ...
    logger->debug("            Updated buttons {}ms", sw.Time());
    ...
    logger->debug("            Status Sent {}ms", sw.Time());
}
...
void xScheduleFrame::CorrectTimer(int rate)
{
    ...
    {
        logger->debug("Timer corrected {}", (rate - __schedule->GetTimerAdjustment()) / (_useHalfFrames ? 2 : 1));
...
void xScheduleFrame::DoAction(wxCommandEvent& event)
{
    ...
    LOG_DEBUG("    Thread switched command = %s", (const char*)amd->_command.c_str());
...
void xScheduleFrame::UpdateUI(bool force)
{
    ...
    logger->debug("        Status updated {}ms", sw.Time());
    ...
    logger->debug("        Web request status updated {}ms", sw.Time());
    ...
    logger->debug("        Managed output to lights {}ms", sw.Time());

    ModeToUI();

    logger->debug("        Updated mode {}ms", sw.Time());
    ...
    logger->debug("        Updated ping status {}ms", sw.Time());
