...
{
    wxConfigBase* config = wxConfigBase::Get();
    spdlog::debug("Config: AppName '{}' Path '{}' Entries {} Groups {} Style {} Vendor '{}'.", (const char*)config->GetAppName().c_str(), (const char*)config->GetPath().c_str(), (int)config->GetNumberOfEntries(), (int)config->GetNumberOfGroups(), config->GetStyle(), (const char*)config->GetVendorName().c_str());
...
void xSMSDaemonFrame::RefreshList()
{
    ...
    if (_smsService != nullptr)
    {
        ...
        if (msgs.size() > 0)
        {
            ...
            {
                spdlog::debug("Deleting {} rows", (int)Grid1->GetNumberRows());
                Grid1->DeleteRows(0, Grid1->GetNumberRows());
            }
            spdlog::debug("Adding {} rows", (int)msgs.size());
            for (auto it : msgs)
            {
