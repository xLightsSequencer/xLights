...
public:
    ...
    {
        spdlog::debug("PlayListRDS in thread.");

        spdlog::info("RDS: PS '{}' DPS '{}'.", (const char*)_stationName.c_str(), (const char*)_text.c_str());

        if (_commPort == "")
        {
            spdlog::warn("RDS: No comm port specified.");
            return nullptr;
        }
        ...
        if (errcode < 0)
        {
            spdlog::warn("RDS: Unable to open serial port {}. Error code = {}", (const char*)_commPort.c_str(), errcode);
            delete serial;
            return nullptr;
        }

        spdlog::debug("Serial port open {}, {} baud, {}.", (const char*)_commPort.c_str(), 19200, serialConfig);
        ...
        spdlog::debug("Serial port closed.");
        spdlog::debug("PlayListRDS thread done.");
...
void PlayListItemRDS::Dump(unsigned char* buffer, int buflen)
{
    ...
    spdlog::debug((const char*)debug.c_str());
}
...
void PlayListItemRDS::Frame(uint8_t* buffer, size_t size, size_t ms, size_t framems, bool outputframe)
{
    ...
    {
        spdlog::warn("Kicking off RDS thread.");
