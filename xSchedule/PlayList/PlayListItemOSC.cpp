...
PlayListItemOSC::PlayListItemOSC() :
    PlayListItem() {
    _type = "PLIOSC";
    _ip = "";
    _path = "";
...
void PlayListItemOSC::Frame(uint8_t* buffer, size_t size, size_t ms, size_t framems, bool outputframe) {
    if (ms >= _delay && !_started) {
        ...
        LOG_INFO("Sending OSC Event to %s:%d %s", (const char*)_ip.c_str(), _port, (const char*)_path.c_str());
        ...
        if (packet.IsOk()) {
            ...
            LOG_ERROR("OSC Packet not sent as it was invalid.");
        }
    }
}
