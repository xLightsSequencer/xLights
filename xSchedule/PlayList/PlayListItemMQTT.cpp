...
public:
    ...
    {
        spdlog::debug("PlayListMQTT in thread.");
        ...
        spdlog::debug("PlayListMQTT Connecting to broker {}:{}.", _brokerIP, _port);
        if (client.Connect(addr, false) || client.WaitOnConnect(0, 500))
        {
            spdlog::debug("    PlayListMQTT Connected to MQTT Broker.");
            uint8_t buffer[1444];
            ...
            {
                spdlog::debug("    PlayListMQTT Response received.");
                if ((buffer[0] & 0xF0) >> 4 == 2)
                {
                    spdlog::debug("    PlayListMQTT Connected ok.");
                    int used = 0;
                ...
                {
                    spdlog::error("PlayListMQTT Illegal response from MQTT broker for connect.");
                }
            }
            else
            {
                spdlog::error("PlayListMQTT No response from MQTT broker for connect.");
            }
            spdlog::debug("    PlayListMQTT Disconnecting from MQTT Broker.");
            client.Close();
        }
        else
        {
            spdlog::error("PlayListMQTT Unable to connect to MQTT broker.");
        }

        spdlog::debug("PlayListMQTT thread done");
...
void PlayListItemMQTT::Frame(uint8_t* buffer, size_t size, size_t ms, size_t framems, bool outputframe)
{
    if (ms >= _delay && !_started)
    {
        ...
        spdlog::info("Sending MQTT Event to {}:{} {}", _brokerIP, _port, _topic);
