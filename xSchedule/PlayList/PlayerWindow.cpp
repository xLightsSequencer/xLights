...
PlayerWindow::PlayerWindow(wxWindow* parent, bool topMost, wxImageResizeQuality quality, int swsQuality, wxWindowID id, const wxPoint& pos, const wxSize& size)
{
    ...
    int x, y;
    GetPosition(&x, &y);
    spdlog::info("Player window created location ({}, {}) size ({}, {}) Quality: {}.", x, y, w, h, VirtualMatrix::DecodeScalingQuality(quality, swsQuality));
}
...
bool PlayerWindow::PrepareImage()
{
    if (_imageChanged) {
        if (_mutex.try_lock_for(std::chrono::milliseconds(1))) {
            ...
            if (_inputImage.IsOk()) {
                spdlog::debug("Updating Player Window image");
                ...
                spdlog::debug("Player Window image updated {}ms", sw.Time());
            }
            _mutex.unlock();
        }
        else {
