...
public:
    ...
    void GetMode(int& mode, REMOTEMODE& remote) const {
        mode = _mode;
        remote = _remoteMode;
    }
    void ToggleMute();
    ...
    int GetVolume() const;
    void AllOff();
    ListenerManager* GetListenerManager() const {
        return _listenerManager;
    }
    int GetManualOutputToLights() const {
        return _manualOTL;
    }
    void ManualOutputToLightsClick(xScheduleFrame* frame);
    bool IsScheduleActive(Schedule* schedue);
    std::list<RunningSchedule*> GetRunningSchedules() const {
        return _activeSchedules;
    }
    const SyncManager* GetSyncManager() const {
        return _syncManager.get();
    }
    int GetTimerAdjustment() const {
        return _timerAdjustment;
    }
    ...
    void SetTimerAdjustment(int timerAdjustment) {
        _timerAdjustment = timerAdjustment;
    }
    PlayList* GetPlayList(int id) const;
    PlayList* GetBackgroundPlayList() const {
        return _backgroundPlayList;
    }
    std::list<PlayList*> GetEventPlayLists() const {
        return _eventPlayLists;
    }
    ...
    std::list<OutputProcess*>* GetOutputProcessing() {
        return &_outputProcessing;
    }
    void WebRequestReceived() {
        _webRequestToggle = !_webRequestToggle;
    }
    ...
    Command* GetCommand(std::string command) const {
        return _commandManager.GetCommand(command);
    }
    ...
    bool IsCurrentPlayListScheduled() const {
        return _immediatePlay == nullptr && GetRunningPlayList() != _queuedSongs;
    }
