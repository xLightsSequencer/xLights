...
the field rig without unwrapping a laptop. The other upload items
(Bulk Upload, HinksPix) stay P2 / P3 — FPP is the wedge.
...
| EX-6 | MultiControllerUploadDialog — controller checklist + log + right-click filters; covers Falcon, WLED, PowerDMX, etc. **Tools → Bulk Controller Upload** on desktop. | P2 | M |
| EX-7 | HinksPixExportDialog — vendor-specific HSEQ format, master+2 slaves, playlists, schedule grid, USB drive export. **Tools → HinksPix Export** on desktop. | P3 | XL |
...
- Bulk upload / HinksPix / drag-drop port mapping are each
  multi-week sprints; no concrete user request has landed.
