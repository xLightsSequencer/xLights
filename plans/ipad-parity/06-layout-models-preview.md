...
| SubModel dimming curve (brightness) | dialog | ✅ | ✅ | parity | P3 | medium | feasible | Desktop SubModelsDialog "Set Dimming Curve" action (brightness-only) writes SubModel dimmingInfo; serialized as `<subModel><dimmingCurve>` and applied at render by stamping each submodel's curve onto the parent's physical nodes (`Model::ApplySubModelDimmingToNodes` → `NodeBaseClass::nodeDimmingCurve`, strongest dim wins on overlap), so the dim is permanent regardless of which effect/model paints the channels; previews reverse it. Two enable switches (both default on, gate the stamping in core so desktop/iPad/headless agree): per-model `Model::SetSubModelDimmingEnabled` (rgbeffects `SubModelDimmingEnabled` attr; desktop SubModelsDialog "Enable Dimming Curves" checkbox, iPad Toggle in SubModelListSheet via subModelDimmingEnabledForModel:) and per-sequence `SequenceElements::SetSupportsSubModelDimming` (xsq `SubModelDimming` attr; desktop Sequence Settings "Enable SubModel Dimming Curves" checkbox, iPad Toggle in SequenceSettingsSheet via sequenceSupportsSubModelDimming). Core (Model + serializer) auto-applies to iPad. iPad exposes the per-submodel value as a Brightness slider in SubModelDetailEditor (LayoutEditorView.swift), threaded through submodelDetailsForModel:/replaceSubModelsOnModel: via SubModelEntry.dimmingBrightness. Issue #6704. |
