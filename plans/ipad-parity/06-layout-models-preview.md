...
| Node Layout (node-grid visualization) | dialog | ✅ | ✅ | parity | P2 | hard | feasible | Desktop ShowNodeLayout/ChannelLayoutDialog (LayoutPanel.cpp:8040) has 10–300% node-grid-only slider/Ctrl+wheel zoom and a fit-to-window action. iPad NodeLayoutSheet (NodeLayoutSheet.swift) — zoom/pan Canvas of node numbers at buffer coords, backed by `nodeLayout(forModel:)` (XLSequenceDocument.mm) over InitRenderBufferNodes. Launched from Model Data section (LayoutEditorView.swift). |
| Wiring View (model node wiring) | dialog | ✅ | 🟡 | parity | P2 | hard | feasible | Desktop ShowWiring/WiringDialog (LayoutPanel.cpp:8050). iPad NodeLayoutSheet `wiring:true` — per-string node path, numbered, front/rear flip. Desktop's print + DXF export + theme/multi-light render options stay desktop-only (deep-dialog cluster). |
...
| Open Faces/States/SubModels editor by double-click | gesture | ✅ | 🟡 | parity | P3 | easy | feasible | Desktop 2026-07 (PR #6740): double-clicking a model in the layout preview opens the unified `ModelDefinitionsDialog` on the SubModels tab — `LayoutPanel::OnPreviewLeftDClick` re-runs a click to select the model under the cursor then calls `EditSubmodels()` (LayoutPanel.cpp; groups/submodels/empty space fall through to clear-selection). Same editors are already reachable on iPad from the Model Data section (SubModelListSheet / FaceStateEditorSheet, see rows above); a matching double-tap shortcut isn't wired because iPad preview double-tap is already bound to reset-camera (see "Preview reset / fit view"). Launch-gesture convenience only — no data gap. |
...
| Replace model(s) with this model | menu | ✅ | ✅ | parity | P3 | hard | feasible | Desktop ID_PREVIEW_REPLACEMODEL / ReplaceModelDialog (#4462, rewritten to multi-target): source model + filtered checkbox target list, options keep-start-channel / keep-submodels / keep-size+pos + Model-Groups mode (keep / replace-with-source / merge). iPad: bridge `replaceModels:withSource:keepStartChannel:keepSubmodels:keepSizePosition:groupMode:forDocument:` (XLMetalBridge.mm) — clones source via XmlSerializer per target, copies start-channel/controller/SR + submodels (SubModel copy ctor) + size/pos/rotation per the flags, atomic name swap, deletes old. Group-mode reconciliation is the shared wx-free `ModelManager::ReconcileReplacedModelGroups` (src-core), called by both platforms. ReplaceModelSheet (LayoutEditorView.swift, searchable multi-select + 3 toggles + Model-Groups picker) launched from the InlineModelActionBar ↻ button. |
...
| Import Previews/Models/Groups/Viewpoints (from RGBeffects) | menu | ✅ | ❌ | ipad-missing | P3 | hard | feasible | Desktop ID_PREVIEW_IMPORTMODELSFROMRGBEFFECTS. iPad: no UI. Desktop dialog gained a live filter box (#6635, `ImportPreviewsModelsDialog`); no new gap — the whole dialog is still iPad-missing, so the filter comes along whenever this is built. Dialog now also imports named Viewpoints (`ViewpointMgr::ImportCameraFromNode`) as a third selectable item type alongside models/groups; same "whenever this is built" note applies. |
...
| Models and Groups in separate list tabs | tab/panel | ✅ | ✅ | parity | P2 | medium | feasible | **Desktop caught up to the iPad (2026-07, PR #6690).** Desktop split the single Groups/Models tree into two `wxTreeListCtrl`s on their own notebook pages, each with its own filter box and saved column order (`LayoutPanel::CreateTreeListCtrl`, `refreshOneModelList`). The iPad has always had separate Models and Groups rosters (`LayoutEditorView.swift`). |
| Empty-state placeholder in the settings pane | tab/panel | ✅ | ✅ | parity | P3 | easy | feasible | **Desktop caught up to the iPad (2026-07, PR #6690).** With nothing selected the desktop settings pane shows a titled "No model/group selected" hint (`LayoutPanel::ShowSettingsEmptyState`) instead of an empty property grid — matching the iPad's placeholder detail pane. |
| Controller list (add/edit/delete/reorder) | tab/panel | ✅ | ✅ | parity | P2 | hard | feasible | Desktop Layout tab → Controllers page (was the standalone Setup tab until PR #6690); iPad Layout sidebar Controllers tab (addController, reorder). Same surface now. |
...
| Faces/States Bulk Color Change (multi-row) | dialog | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop grid right-click "Bulk Color Change..." sets one color across every selected node-range row in a single pick: `ModelFacesPanel::OnGridPopup` (ModelFacesPanel.cpp:1551, `FACES_DIALOG_BULK_COLOR_CHANGE`) and the identical `ModelStatesPanel::OnGridPopup` (ModelStatesPanel.cpp:1263, `STATE_DIALOG_BULK_COLOR_CHANGE`), gated on a 2+ row multi-select. iPad's FaceStateEditorSheet edits attributes one row at a time; no multi-select + single-picker path. |
| Faces tab: live State-outline preview overlay | dialog | ✅ | ❌ | ipad-missing | P3 | medium | feasible | Desktop `ModelFacesPanel::ApplyStateOutlineOverlay` (ModelFacesPanel.cpp) lets you pick a NodeRange state from a dropdown while editing Faces, and paints that state's per-node colors onto the model preview as a reference overlay (reads the States tab's in-flight, not-yet-saved edits via a cross-tab callback, so it stays correct even before the dialog is saved). iPad's FaceStateEditorSheet has no equivalent overlay/reference-state picker. |
| States tab: sequential state-playback preview | dialog | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop `ModelStatesPanel::OnPlayStatesClick` (ModelStatesPanel.cpp, `Button_PlayStates`) cycles the model preview through each defined NodeRange state in name order on a timer (`_stateAnimTimer`, speed via `Spin_StateSpeed`), so you can watch a sequence of states play back without manually re-selecting rows. iPad's FaceStateEditorSheet has no playback control. |
| Faces/States: pick nodes from in-session (unsaved) SubModels | dialog | ✅ | ❌ | ipad-missing | P3 | medium | feasible | Desktop's unified Model Definitions dialog wires a cross-tab callback (`ModelDefinitionsDialog` → `SetSubModelCallbacks`, consumed by `ModelFacesPanel::ImportSubmodel` / the States tab's equivalent) so the Faces/States "Import SubModel" node picker offers SubModels as edited in the SubModels tab *this session*, even before Save. iPad edits Faces/States and SubModels in separate sheets over the already-saved model, so a picker there only ever sees previously-saved SubModels - a different (sequential, always-saved) workflow rather than a like-for-like gap. |
