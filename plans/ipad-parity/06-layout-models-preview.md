...
| SubModel dimming curve (brightness) | dialog | ✅ | ✅ | parity | P3 | medium | feasible | Desktop SubModelsDialog "Set Dimming Curve" action (brightness-only) writes SubModel dimmingInfo; serialized as `<subModel><dimmingCurve>` and applied at render by retargeting the submodel's render-node `model` back-pointer to itself when it carries a curve (SubModel::ApplyDimmingNodeOwnership), so PixelBuffer's per-node `GetDimmingCurve()` picks it up. Core (SubModel + serializer) auto-applies to iPad. iPad exposes it as a Brightness slider in SubModelDetailEditor (LayoutEditorView.swift), threaded through submodelDetailsForModel:/replaceSubModelsOnModel: via SubModelEntry.dimmingBrightness. Issue #6704. |
