...
| Last Input/Output Upload timestamp | dialog/panel | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop shows two read-only fields at the bottom of the property pane (`Controllers_PropertyEditor` in `src-ui-wx/app-shell/TabSetup.cpp`). Timestamps persisted in `GetXLightsConfig()` (JSON app settings, not network file) from both manual upload (`UploadInputToController`/`UploadOutputToController` at `src-ui-wx/app-shell/TabSetup.cpp`) and FPP Connect (`src-ui-wx/controllers/FPPConnectDialog.cpp`). `FormatTimestamp()` centralized in `src-core/utils/UtilFunctions.h`. iPad controller detail pane has no equivalent. |
