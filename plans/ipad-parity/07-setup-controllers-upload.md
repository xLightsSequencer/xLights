...
| Last Input/Output Upload timestamp | dialog/panel | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop shows two read-only fields at the bottom of the property pane (`xLightsFrame::SetControllersProperties` in `src-ui-wx/app-shell/TabSetup.cpp`). Stored as `ExtraProperty` via `Controller::SetExtraProperty`. iPad controller detail pane has no equivalent. |
