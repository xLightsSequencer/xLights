...
| Last Input/Output Upload timestamp | dialog/panel | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop shows two read-only fields in property pane showing last upload time or "Never" (`src-ui-wx/app-shell/TabSetup.cpp:2053-2069`). Stored as `ExtraProperty` via `Controller::SetExtraProperty`. iPad controller detail pane has no equivalent. |
