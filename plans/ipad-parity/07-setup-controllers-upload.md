...
| Last Input/Output Upload timestamp | dialog/panel | ✅ | ✅ | parity | P3 | easy | feasible | Desktop & iPad show two read-only fields at the bottom of the property pane. Persisted per-show via `xlights_local.json` (`ReadControllerTimestamp`/`WriteControllerTimestamp` in `src-core/utils/UtilFunctions.h`) — no `NetworkChange()` call, so uploading never marks the show dirty. Recorded from manual upload (`TabSetup.cpp`), FPP Connect (`FPPConnectDialog.cpp`), and iPad bridge (`XLSequenceDocument.mm:runUpload:`). Stale entries pruned on every write using the current controller list. iPad detail pane renders timestamps via `TimestampRow`. |
