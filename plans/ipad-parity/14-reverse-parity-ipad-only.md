...
| Edge auto-scroll (auto-pan) while dragging/resizing/fading an effect | gesture | ✅ | ✅ | parity | P2 | medium | feasible | Desktop now runs a wxTimer-driven edge auto-scroll during resize/move drags: `EffectsGrid::UpdateEdgeAutoScroll`/`OnAutoScrollTimer` pan `PanelTimeLine` when the cursor enters the `AUTO_SCROLL_MARGIN` band, with a proximity speed ramp, re-running the active Resize/move each tick (mirrors the iPad margin/speed ramp). |
| Drag-to-move snap-to-grid for layout models / view objects | capability | ✅ | ✅ | parity | P2 | easy | feasible | Desktop adds a 'Snap To Grid' bool to the layout preview property grid (`LayoutPanel.cpp` `2DSnapToGrid`, persisted via `xLightsFrame::Set/GetSnap2DGrid`); on 2D body-drag mouse-up `SnapSelectedModelsToGrid` quantises each selected model's centre to `GetDisplay2DGridSpacing()`. |
...
| Copy current palette string to clipboard | context-menu | ✅ | ✅ | parity | P3 | easy | feasible | Desktop adds 'Copy Palette String' to the ColorPanel palette menu (`ID_MNU_COPY` → `ColorPanel::CopyPaletteString` writes `GetCurrentPalette()` to `wxTheClipboard`), round-tripping with the existing Import Palette text dialog. iPad `ColorPaletteView.swift:133`. |
...
2. ~~**Edge auto-scroll while dragging (desktop, P2)**~~ — *Done*: wxTimer-driven
