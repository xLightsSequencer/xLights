...
| Moving Head: "Link" tab (snap end Pan/Tilt to next effect + force Dimmer to 0) | panel | ✅ | ➖ | ipad-missing | P2 | medium | feasible | Desktop new `MovingHeadPanel.h:69` (`CheckBox_MHLinkToNext`) / `MovingHeadPanel.cpp` (`SyncLinkToNext`, `ApplyLinkedHeadPosition`) — panel-time-only "Link" tab that reads the next Moving Head effect on the row via `CallOnEffectAfterSelected` and, per active head, snapshots its start Pan/Tilt (`MovingHeadEffect::GetHeadStartPosition`, `MovingHeadEffect.h:56`/`.cpp`) into this effect's own end-of-effect `Pan:`/`Tilt:` commands, plus forces the head's `Dimmer:` curve's last handle to 0. Shared `src-core/effects/MovingHeadEffect.{h,cpp}` parsing/position-calculation helpers (`GetValueCurvePosition`, `CalculatePosition`, `CalculatePathPositions`, `CalculatePatternPositions`) were made `static` and reused by the new helper — render path (`Render`/`RenderMovingHead`) behavior is unchanged, so the computed command strings render identically on iPad once written. **Deferred:** needs new SwiftUI row UI (checkbox + live preview text) plus a bridge method to read the next effect's per-head `TEXTCTRL_MHx_Settings` and to splice the computed Pan/Tilt + forced-zero Dimmer handle into this effect's own per-head settings — not a small addition, no existing MovingHead row view covers "read the next effect on the row." |
...
| Moving Head: Pattern tab (parametric pan/tilt shapes) | panel | ✅ | ❌ | ipad-missing | P2 | medium | feasible | Desktop `MovingHeadPanel.cpp:110-112` (`ID_CHECKBOX_MHPatternEnable`, `ID_CHOICE_MHPattern`, `ID_STATICTEXT_MHPatternShape`) generates pan/tilt from parametric shapes (Circle, Eight, Diamond, Lissajous, …) modelled on QLC+ EFX, with a value-curve-animatable rotation. No `Lissajous` / `MHPattern` reference in `src-iPad/`; the iPad Moving Head panel has fixture/color/dimmer rows and the path canvas but no pattern generator. Render side is shared core, so pattern-authored effects should already play back — worth confirming alongside the panel work. |
| Moving Head: Enable Shutter checkbox | panel | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop Control tab checkbox (RGB and colour wheel) holds the shutter open at the model's Shutter On Value; Auto Shutter still takes over when active. iPad's only `Shutter` references are the DMX *model* property block (`XLSequenceDocument.mm:8064` `AppendDmxShutterProps`) — that's the fixture definition, not the effect's new toggle. Likely a one-row addition to the Moving Head effect panel once the setting key is confirmed. |
