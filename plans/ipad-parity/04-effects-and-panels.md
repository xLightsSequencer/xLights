...
| Effect Symbols (reusable linked effects) | dialog + grid | ✅ | ❌ | ipad-missing | P2 | hard | feasible | Desktop #2671: define an effect once and link any number of effects to it, editing one propagates to all linked; symbols round-trip through the `.xsq` and a "Convert All Symbols to Effects" action detaches them. Desktop surface = `src-ui-wx/EffectSymbolDialog.cpp` + `MenuItem_EffectSymbols` (`xLightsMain.h:986`). No `EffectSymbol` references anywhere in `src-iPad/`. The storage/propagation lives in the shared sequence model, so an `.xsq` authored with symbols opens on iPad — verify the iPad renders linked effects correctly and doesn't silently break the link on edit before treating this as UI-only work. |
