...
> import, path reorder, and curve-segment authoring all landed). As of
> **2026-07-25 the Moving Head panel is fully at parity**: the parametric
> **Pattern** tab (10 QLC+-style shapes + 11 params + rotation value curve), the
> **Link** tab (snap the end position to the next effect and go dark), and the
> **Enable Shutter** checkbox all landed, and the long-inert **Auto Shutter**
> toggle now actually writes `AutoShutter: true`. The **DMX
...
| Moving Head: "Link" tab (snap end Pan/Tilt to next effect + force Dimmer to 0) | panel | ✅ | ✅ | parity | P2 | medium | feasible | Desktop `MovingHeadPanel.h:104` (`CheckBox_MHLinkToNext`) / `MovingHeadPanel.cpp` (`SyncLinkToNext`, `ApplyLinkedHeadPosition`) — reads the next Moving Head effect on the row via `CallOnEffectAfterSelected` and, per active head, snapshots its start Pan/Tilt (`MovingHeadEffect::GetHeadStartPosition`, `MovingHeadEffect.h:56`) into this effect's own `Pan:`/`Tilt:` commands, plus forces the head's `Dimmer:` to flat zero. **Landed 2026-07-25.** iPad: a `Link` tab (`MovingHead.json`) with the `MHLinkToNext` checkbox and a `MHLinkRow` custom row (`MovingHeadPresetViews.swift` `MovingHeadLinkRowView`) showing the same per-head preview desktop puts in `StaticText_MHLinkPreview`, plus a **Re-sync** button desktop lacks (desktop only re-syncs on the checkbox click, so an edit to the *next* effect goes unnoticed). Bridge `syncMovingHeadLinkToNextForRow:atIndex:` walks to `effectIndex + 1` on the same layer, calls the shared core `GetHeadStartPosition`, and splices via `applyMHLinkedHeadPosition` — a verbatim port of the desktop `linkedPositionOverrideSettings` strip list, so a linked head unconditionally ends where the next effect starts however this effect drove it. Head selection mirrors desktop: a single fixture model uses its own `GetFixtureVal()`, a group falls back to every head with settings (iPad derives its fixture buttons from those same strings, so desktop's checkbox union collapses to the same set). Returns `{status, lines}` so the row can distinguish no-next-effect / next-not-a-Moving-Head / no-active-heads. Pattern + Pathing grey out while linked (`visibilityRules`), matching desktop's `UpdateLinkTabState`, and `syncMHPatternCommands` refuses to re-emit the pattern block while linked. Undo is stronger than desktop's: Link is a destructive rewrite, so the view model snapshots all eight `MH<n>_Settings` strings (`registerMovingHeadFixtureUndo`) and restores them verbatim in the same undo group as the checkbox. |
...
| Fire: Style (Old/New Render Method) + GPU flame advance | panel + render | ✅ | ✅ | parity | P2 | done | feasible | Core-only (2026-07-25), auto-applied to both platforms. The panel row comes free — Fire's panel is JSON-driven (`resources/effectmetadata/Fire.json` → `JsonEffectPanel` on desktop, `EffectMetadata.swift` on iPad), so the new `Fire_Style` choice renders on iPad with no Swift change. The render side is shared `src-core`: "New Render Method" advances the flame temporally (row y from the PREVIOUS frame's rows y-1/y-2) so the whole grid updates at once — `FireFunctions.ispc` on CPU, `FireFunctions.metal` + `MetalFireEffect` on Metal, which the iPad links too. The classic method's in-frame vertical dependency is strictly serial in y and has no GPU path on either platform. New effects default to "New Render Method" on both platforms; `adjustSettings` pins pre-2026.15 sequences to "Old Render Method", so existing iPad and desktop sequences are unchanged. GPU paths: `FireFunctions.metal` + `MetalFireEffect` (Apple, incl. iPad) and `FireAdvanceEffect.comp`/`FireDrawEffect.comp` + `VulkanFireEffect` (Linux/Windows), all bit-identical to the ISPC CPU path. |
...
| Convert selected effects to a different type | context-menu | ✅ | ✅ | parity | P2 | hard | feasible | Desktop `EffectsPanel.cpp:348` right-click → `wxSingleChoiceDialog` over the effect list → `MainSequencer::ConvertSelectedEffectsTo` → `EffectsGrid::ConvertSelectedEffectsTo`, which retypes every selected effect, drops its `E_*` settings and seeds the new type's panel defaults. **Landed 2026-07-25.** iPad: **Convert To…** on the effect long-press menu opens `ConvertEffectTypeSheet` — a searchable list rather than a flat 57-item menu, which is unusable on touch; the current type is ticked and non-selectable. `SequencerViewModel.convertSelectedEffects(to:)` drops `E_*`, keeps the palette and the shared Buffer / Blending panels, and drives the existing `restoreEffectInRow:atIndex:type:settings:palette:symbolId:` bridge — which unlinks before writing, so retyping a symbol-linked effect can't propagate out to every sibling, then re-links. One undo step covers the whole multi-selection. **One deliberate difference:** desktop writes the new type's defaults into the settings string; iPad leaves `E_` empty, which reads identically to the renderer (an absent key *is* its default) and is exactly the state a freshly-dropped effect has here — so a converted effect and a new one are indistinguishable, and the `.xsq` stays smaller. |
...
| Effect Symbols (reusable linked effects) | dialog + grid | ✅ | ✅ | parity | P2 | medium | feasible | Desktop #2671 (`919340220`): define an effect once, link many, edit one and all update. **Investigated 2026-07-25 — the storage layer is fully shared and the iPad is data-safe; what it lacks is the UI.** (1) *Persistence is core:* `EffectSymbol` / `EffectSymbolManager` live in `src-core/render/` and the manager is a member of `SequenceElements`, so the iPad owns one. Save writes `<EffectSymbols>` before `ElementEffects` plus a per-effect `linkedSymbol` attribute (`SequenceFile.cpp:1316,1402-1407`); load parses them and re-links (`SequenceElements.cpp:742-745,848-849`). The iPad saves through `SequenceFile::Save` and loads through `LoadSequenceElements`→`LoadSequencerFile`, so an iPad open→save **round-trips symbols and links intact**. (2) *Editing propagates:* propagation is driven from `Effect::IncrementChangeCount()` (`Effect.cpp:504-523`, thread_local recursion guard), and every mutator calls it — `SetSetting:362`, `SetPalette:821`, `SetEffectName:343`, `PaletteMapUpdated:911`. The iPad's edit paths (`setEffectSettingValue:` `.mm:11283-11298`, `replaceEffectSettings:` `.mm:11676-11681`) go through those, so editing a linked effect on iPad updates the symbol and every sibling exactly as on desktop. Undo is safe too — `UndoManager.cpp:376-380` restores via the same mutators, so the restore re-propagates. `~Effect()` unregisters from the manager (`Effect.cpp:254-263`), so no dangling pointers when the iPad deletes. (3) *Fixed here:* `Effect::GetSettingsAsString()` appends `X_LinkedSymbolId` for a linked effect (`:553-557`), and desktop consumes it with `HandlePastedSymbolLink()` at all five AddEffect-from-settings-string sites. The iPad had **zero** callers, so paste, undo-of-delete **and move-to-another-layer** (the only three callers of the bridge's `addEffectToRow:`) each dropped the link and persisted a stray `X_LinkedSymbolId` into the `.xsq` — never cleaned up, since the strip at `Effect.cpp:984` only runs inside `LinkToSymbol`. Now called in `addEffectToRow:` (`.mm:11697`), which is a no-op when the key is absent. (4) *Indicator landed:* the iPad grid now draws the same cyan (0,200,255) 6px top-right corner triangle desktop does (`EffectsGrid.cpp:7638-7644`) — `EffectsMetalGridView` draws it per visible effect from a new `EffectStateLookup.isLinkedToSymbol` closure over the bridge's `effectIsLinkedToSymbolInRow:atIndex:`, skipped on effects narrower than the indicator, and the 🔒 glyph shifts left when an effect is both locked and linked so both stay readable. (5) *Management UI landed:* the effect long-press menu gained an **Effect Symbol** submenu — Create Symbol from Effect… (name prompt, rejecting empty/duplicate names as desktop does), Unlink from Symbol, and Link to Symbol ▸ listing every symbol as `name (type)` — mirroring `EffectsGrid.cpp:614-633`, and acting on the whole multi-selection like desktop. Tools ▸ **Effect Symbols…** (also in the command palette) opens `EffectSymbolsSheet`: rename / delete per symbol with linked counts, plus **Convert All Symbols to Effects** with the same confirm-then-report flow as desktop's `xLightsMain.cpp:6106`. Bridge: `effectSymbols`, `linkedSymbolIdInRow:atIndex:`, `createSymbolNamed:fromEffectInRow:atIndex:`, `linkEffectInRow:atIndex:toSymbolId:`, `unlinkEffectFromSymbolInRow:atIndex:`, `renameSymbolId:toName:`, `deleteSymbolId:`, `convertAllSymbolsToEffects`, and `restoreEffectInRow:atIndex:type:settings:palette:symbolId:` for undo. All of it drives the shared `EffectSymbolManager` — no new logic. Undo covers link/unlink/create: linking can change an effect's *type* as well as its settings, so the restore carries type + settings + palette + previous symbol id, and it **unlinks before writing the settings** so an undo can't propagate back into the symbol and clobber every sibling. **Parity reached** — the one desktop surface not mirrored is the standalone `EffectSymbolDialog`'s browse-only list, folded into the same sheet. |
| Moving Head: Pattern tab (parametric pan/tilt shapes) | panel | ✅ | ✅ | parity | P2 | medium | feasible | Desktop `MovingHeadPanel.cpp:417-547` (`ID_CHECKBOX_MHPatternEnable`, `ID_CHOICE_MHPattern`, eleven `ID_SLIDER_MHPattern*`) generates pan/tilt from parametric shapes (Circle, Eight, Line, Line2, Diamond, Square, SquareChoppy, SquareTrue, Leaf, Lissajous) modelled on QLC+ EFX, with a value-curve-animatable rotation. **Landed 2026-07-25.** The generator itself is already shared core (`MovingHeadEffect::CalculatePatternPoint` / `CalculatePatternPositions`, `.cpp:679-767`) and the commands it reads (`Pattern`, `PatternWidth`…`PatternYPhase`, `PatternRotation VC`) are parsed in `RenderMovingHead` — only the authoring panel was missing, so iPad needed no render work. iPad: a `Pattern` tab in `MovingHead.json` with all thirteen controls (the Rotation slider carries `valueCurve` with the core's 0..360 limits) plus `visibilityRules` that grey the four Lissajous-only fields off any other shape — desktop's `UpdatePatternControlState` — and grey the whole block off when the enable checkbox is clear. Bridge `syncMovingHeadPatternForRow:atIndex:` re-fans the controls into every active fixture's `MH*_Settings`, mirroring `UpdatePatternSettings` (strip the whole `Pattern*` block, re-append only while enabled); routed from `SequencerViewModel.syncMovingHeadCommands` on both the forward edit **and** the undo restore. Defaults match the renderer's own fallbacks. Desktop `RecallSettings` (`:2916-2945`) reads the same commands back, so an iPad-authored pattern drives the desktop panel unchanged. Not ported: desktop greys Head Spread off for a single model — harmless on iPad, where the field is a no-op for one head. |
| Moving Head: Enable Shutter checkbox | panel | ✅ | ✅ | parity | P3 | easy | feasible | Desktop Control-tab checkbox (`ID_CHECKBOX_MHShutterEnable`) writes `Shutter: On` into every checked fixture (`MovingHeadPanel::UpdateColorSettings:1808`), holding the shutter open at the model's Shutter On Value; Auto Shutter still takes over when active. **Landed 2026-07-25** as `MHShutterEnable` on the iPad Control tab, re-fanned by bridge `syncMovingHeadShutterForRow:atIndex:`. The same sweep fixed the pre-existing **Auto Shutter** gap: iPad already rendered the `AUTO_SHUTTER` checkbox but nothing ever wrote `AutoShutter: true` into `MH*_Settings`, so the toggle was inert. It now emits alongside a `Wheel` colour exactly as desktop's wheel branch does, and picking/clearing a wheel colour re-evaluates it. |
...
- **Convert selected effects to a different type. (Done.)** `ConvertEffectTypeSheet`
  + `SequencerViewModel.convertSelectedEffects(to:)`; see the scorecard row.
