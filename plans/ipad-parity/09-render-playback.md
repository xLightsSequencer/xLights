...
| Render abort / cancel | internal / keybind | ✅ | ✅ | parity | P1 | medium | feasible | Desktop `AbortRender` + `CANCEL_RENDER` keybind; iPad `abortRenderAndWait`. iPad has no user-facing cancel button (auto on memory pressure). The abort is best-effort on both and its result is now load-bearing: shared `xLightsShowContext::CloseSequence` is `[[nodiscard]] bool`, and `iPadRenderContext::OpenSequence`/`RenderAll` refuse to replace the sequence or start a second render pass when the drain fails rather than rebuilding buffers under live workers. |
