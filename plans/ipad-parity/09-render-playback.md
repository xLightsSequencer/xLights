...
| Render profiling / diff tooling (`XL_RENDER_PROFILE`, `--fseqcmp` extras) | CLI/env | ✅ | ➖ | n/a | P3 | — | infeasible-idiom | Per-model/per-effect timing dumps and the `--fseqcmp` frame-window / PNG-diff / changed-frame-range options are developer CLI + env-var tooling with no iOS equivalent. Listed so the audit is complete, not as a work item. |
