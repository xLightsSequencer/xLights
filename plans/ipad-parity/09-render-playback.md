...
> separate Pause/Replay-Section toolbar buttons (it folds them into a
> play/pause toggle + a loop-region context menu), and no "Log Render
> State" diagnostic. Conversely the iPad has **richer touch idioms** —
...
> keybinding analogues on desktop. The single most notable *iPad-missing*
> item with real user value is the **continuous waveform/transport
> position slider** for coarse seeking; the iPad relies on tap/scrub on
> the Metal ruler instead. Loop-region and "render just this region" exist
...
| Replay Section / loop selected region | toolbar / keybind | ✅ | ✅ | parity | P2 | medium | feasible | Desktop = Replay-Section toolbar btn (`mLoopAudio=true`) + `PLAY_LOOP` keybind; iPad = B32/B33 loop region + "Play Loop Region" context action. Different UI, same intent. |
...
| Position / seek slider | toolbar | ✅ | ❌ | ipad-missing | P2 | medium | feasible | Desktop HousePreview `SliderPosition` (0–100 scrub). iPad has no coarse slider — only Metal-ruler tap/drag. |
...
| Render progress indicator | dialog / toolbar | ✅ | 🟡 | parity | P2 | easy | feasible | Desktop modal `RenderProgressDialog` (per-job gauges); iPad shows a toolbar spinner only (no per-job bars). |
...
- **Position / seek slider (coarse scrub).** Desktop HousePreview has a
  full-width `SliderPosition` (`HousePreviewPanel.cpp:83`,
  `OnSliderPositionCmdSliderUpdated`) that maps 0–100 across the whole
  sequence for one-gesture coarse seeking. The iPad only offers tap/drag
  on the Metal ruler (`TopChromeMetalGridView` → `onSeek`/`onScrubSeek`),
  which is precise but awkward for "jump roughly two-thirds in" on a long
  song. *Work:* add a `Slider` bound to `playPositionMS / sequenceDurationMS`
  in `SequencerView.toolbar` (or the transport strip), calling
  `viewModel.seekTo`. **Ease: medium** (the bind + a drag-vs-playhead
  sync are the only subtleties; core seek already exists).
...
1. **Position / seek slider on iPad (P2).** Highest-value iPad-missing
   item; coarse seeking on long songs is awkward today. Pure SwiftUI +
   existing `seekTo`. Build first.
2. **Per-job render progress on iPad (P2→polish).** Replace the bare
   toolbar spinner with progress fed from `IRenderProgressSink`-style
   callbacks the core already emits, so users see render advancement on
   big sequences.
