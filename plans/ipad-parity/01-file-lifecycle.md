...
| Settings — Faces tab (sequence-level face definitions) | dialog tab | ✅ | ✅ | parity | P2 | medium | feasible | 2026-07: **landed.** iPad `SequenceFacesTab` in `SequenceSettingsSheet.swift` (segmented "Faces" tab) edits the Matrix/image face defs stored in the `.xsq` (`SequenceFaces` on `SequenceElements`, shared core): face picker + Add/Delete/Rename, Image Placement, a 10-phoneme × 2 (Eyes Open/Closed) image grid (per-cell pick reuses `MediaPickerSheet` + `.mediaRelocationPrompt`), and Import-from-Model. Bridge methods on `XLSequenceDocument` (`sequenceFaces`, `add/remove/rename/setSequenceFaceImage/setSequenceFacePlacement`, `sequenceFaceImportSources`, `importSequenceFaceFromModel:face:`, `reRenderSequenceFaceEffects:`) mirror the desktop `SequenceFacesPanel`, incl. reference-repointing on rename and re-render on edit. Deferred vs desktop: "Download Images" web-fetch dialog and the filename-pattern phoneme auto-fill (both desktop-dialog / wx-helper bound). |
...
| Select Show Folder | menu / sheet | ✅ | ✅ | parity | P1 | easy | feasible | iPad `FolderConfigView` (gate-required before sequences); 'Change Show Folder…' opens the system folder picker, commit via `viewModel.loadShowFolder`. **Landed 2026-07-25:** the chosen folder now goes through desktop's own sanity heuristic (`xLightsFrame::PromptForShowDirectory`, `TabSetup.cpp:605`) — a folder holding `xlights_networks.xml` or `xlights_rgbeffects.xml` is accepted outright, an empty one is accepted as a fine place to start, and anything else raises "Possibly Incorrect Folder" with Use Anyway / Choose Another (desktop's "Folder chosen does not contain xLights show folder files and is not empty" prompt). Dotfiles are ignored when testing for empty, since iOS scatters them. |
...
| Base Show Folder (inheritance) | setup/sheet | ✅ | ✅ | parity | P2 | medium | feasible | Desktop `ShowDirectoriesDialog` (`src-ui-wx/setup/ShowDirectoriesDialog.cpp`, was the Setup tab until PR #6690); iPad FolderConfigView base section + Auto-Update + Update Now. The merge itself is shared core (`OutputManager::MergeFromBase`), so the fix for the same-name/different-type controller crash (a `ControllerSerial`/`ControllerEthernet` downcast of the wrong type) is auto-applied on both platforms. |
| Show/base directory config in a dedicated sheet | setup/sheet | ✅ | ✅ | parity | P3 | easy | feasible | **Desktop caught up to the iPad (2026-07, PR #6690).** Show dir + base show dir + change permanently/temporarily moved off the Setup tab into the modal `ShowDirectoriesDialog`, reached from a folder button on the Layout toolbar; a footer label shows the current show/base folder names. Mirrors the iPad's `FolderConfigView` sheet. 2026-07 (PR #6740): the Layout footer folder label now opens this dialog (was: launched the OS file manager directly), and double-clicking the show/base path labels inside the dialog opens the folder in Finder/Explorer (`ShowDirectoriesDialog.cpp`, `wxLaunchDefaultApplication`). The "open folder in OS file manager" gesture is desktop-only — no iPad equivalent under the iOS sandbox. |
...
| Recall last directory in Open / Save-As | dialog | ✅ | ➖ | ipad-missing | P3 | easy | infeasible-idiom | Desktop #3107 remembers the last-used folder for Open Sequence / Save Sequence As. iPad opens through `UIDocumentPicker` / the show-folder browser, where the system restores the last browsed location itself and the app cannot set it. No work item. |
| First run with no show folder: offer a default at Documents/xLights | first-run | ✅ | ✅ | parity | P2 | easy | feasible | Desktop `xLightsFrame::OfferDefaultShowDirectory` (`TabSetup.cpp:627`) prompts "No show folder is set… create one at Documents/xLights?" with a pick-instead option. **Landed 2026-07-25.** iPad `FolderConfigView` — which *is* the first-run gate — now shows a **Use Default Show Folder** button with the target path beneath it, but only while nothing is set; it creates `<Documents>/xLights` (the app's Files-visible container, so the show lands under "On My iPad ▸ xLights" and syncs when iCloud Drive is on) and reports a failure with the same "please choose a folder instead" fallback. Presented as a button rather than desktop's modal yes/no because the sheet is already on screen asking exactly this question. |
| Show Directories dialog (show + base show folder, pick / reveal) | dialog | ✅ | 🟡 | ipad-weaker | P3 | easy | feasible | Desktop moved show + base-show-folder settings into `ShowDirectoriesDialog` (`src-ui-wx/setup/ShowDirectoriesDialog.cpp`) opened from the Layout tab, with buttons to pick a directory and reveal it in Finder/Explorer. iPad has the same two settings in `FolderConfigView` (show folder + base show folder + unlink) but no reveal-in-Files action. |
| Skip base-show-folder merge when the base is unchanged | core/perf | ✅ | ✅ | parity | P3 | easy | feasible | Desktop skips the automatic base-folder merge of controllers/models/view objects when the base folder has not changed since the last saved sync. The check lives in shared show-loading core, so the iPad's `loadShowFolder` path gets the same skip and the same faster open — no iPad UI surface. |
