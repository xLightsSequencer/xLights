...
| First run with no show folder: offer a default at Documents/xLights | first-run | ✅ | 🟡 | ipad-weaker | P2 | easy | feasible | Desktop `TabSetup.cpp:631` prompts "No show folder is set… create one at Documents/xLights?" with a pick-instead option. iPad gates the whole app behind `AccessGate` / `FolderConfigView` (must pick a folder before sequences open) but never *offers to create* a default, so a first-time user with no folder has to know to make one. Work: an "Create at Documents/xLights" button in the gate. |
