...
> warnings, missing-media report, and the four import options
> (erase/lock/convert-render/faces-into-sequence). The iPad's **timing import** (in
...
| Effect-import channel-mapping wizard (two-pane src/dest tree) | dialog | ✅ | ✅ | parity | P1 | easy | feasible | Desktop `xLightsImportChannelMapDialog`; iPad `ImportEffectsView` mapping panes + search filters. 2026-07-29: the desktop tree's sort comparator was rebuilding both names through `wxString` twice per comparison — fixed in `xLightsImportTreeModel::Compare` (desktop-only code). The shared `NumberAwareStringCompare` (`src-core/utils/UtilFunctions.cpp`) it calls was also made allocation-free (~2.3x); that half is core, so it is auto-applied to every iPad name sort too. |
...
| Open `.xbkp` backup directly | dialog/menu | ✅ | ✅ | parity | P3 | easy | feasible | Desktop Open wildcard accepts `.xbkp`. iPad routing: `handleIncomingSequenceURL` (`XLightsApp.swift:751`) detects a `.xbkp` URL, opens it through the normal `openSequence` path (it IS `.xsq` XML), then sheets an "Opened a Backup File" alert offering **Save As…** (bumps `saveAsRequestToken`); the pre-existing newer-than-`.xsq` autosave *recovery* sheet is unchanged. The previously-blocking registration is now in `macOS/Assets/xLights-iPad/Info.plist` (submodule bump): exported UTI `org.xlights.sequence-backup` (extension `xbkp`, conforms to `org.xlights.sequence`) + a matching `CFBundleDocumentTypes` entry, so Files/iCloud delivers `.xbkp` taps to `.onOpenURL`. Verified present in the built app bundle's `Info.plist`; the Files-tap round trip itself needs a device. |
...
| Import model Matrix faces into the sequence (embedded) | dialog toggle | ✅ | ✅ | parity | P3 | easy | feasible | Desktop checkbox `CheckBox_ImportFacesToSequence` (`xLightsImportChannelMapDialog`) drives `SequencePackage::SetImportFacesToSequence`; core `ImportModelFaceToSequence` (shared, `SequencePackage.cpp`) reads the source model's Matrix `faceInfo` from the imported rgbeffects and stores it in the target `SequenceFaces` with images embedded (bytes from package `_media` or resolved against the source show folder), instead of `ImportFaceInfo`'s add-to-layout. Node/Coro faces fall back to the legacy layout path. iPad = "Faces into sequence" switch in `ImportEffectsView`'s `optionsBar` next to erase/lock/convert-render, passed through `XLImportSession applyImportWithEraseExisting:lock:convertRenderStyle:importFacesToSequence:` to the same `SetImportFacesToSequence`. Both platforms remember the choice across imports (desktop: `ImportFacesToSequence` in the xLights config; iPad: `@AppStorage("import.facesToSequence")`), default on, and both hide the control for non-xLights sources (no rgbeffects — iPad gates on the new `sourceHasRGBEffects`). A loaded desktop mapping file still overrides it for that import. |
...
| Model export: HD upscale option | dialog | ✅ | ✅ | parity | P3 | easy | feasible | Desktop #4228 adds an HD upscale option to model export (`xLightsMain.h:1508` `exportWidth`/`exportHeight`, used at `RenderUI.cpp:537` for 1920×1080). iPad has the same variant on the bridge — `XLSequenceDocument.h:652-653`, "HD-upscale variant … scales the output to `exportWidth`×`exportHeight` (nearest-neighbour integer upscale, black bars)" — surfaced from the row-header export flow (`RowHeaderViews.swift:414`). |
| Scale imported model sizes to the destination ruler | import | ✅ | ❌ | ipad-missing | P3 | medium | feasible | Desktop #6250 scales imported model dimensions to match the destination layout's ruler when the source layout has one, so a model imported from a differently-calibrated show lands at the right real-world size. No ruler-scaling on the iPad import path (`src-iPad/` ruler references are all timeline/preview, not import). The iPad already has ruler-calibrated real-world readouts (theme 06), so the calibration data is available to it. |
| Convert Incompatible Videos: near-lossless when the source is high quality | tool | ✅ | ❌ | ipad-missing | P3 | medium | feasible | Desktop's Convert-Incompatible-Videos now encodes near-lossless when the source was a high-quality video instead of always re-encoding lossy. iPad has no convert-videos tool at all (theme 13 records the missing Convert tool); this refines a tool the iPad doesn't have, so it is folded in rather than tracked separately. |
