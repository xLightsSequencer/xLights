...
| **IE-4** | 08 | **LOR S5 `.loredit` import** (discovery/source-load slice). `LOREdit` relocated to wx-free `src-core/import_export`; desktop `ImportS5` rewired to share it; iPad loads `.loredit` into the existing mapping/auto-map flow. (Full effect-apply is the next slice.) | ✅ iPad + desktop |
| **IE-15** | 08 | **Export Models XLSX** (core + bridge + UI). wx-free `src-core/import_export/ExportModels` (libxlsxwriter) + bridge + Layout-editor export. ⚠️ app-LINK needs `-lxlsxwriter` added to the `xLights-iPad` target `OTHER_LDFLAGS` (in the `macOS` submodule — outside this commit). | ✅ iPad-lib compile |

> **Worktree-agent integration (2026-06-01):** the three items above were each implemented by a **worktree-isolated agent** (branched from the `ipad-parity-p1` commit, self-built), then merged via patches with one small conflict resolved (PRE-1's `SequencerViewModel`/`XLightsCommands` overlap with Color Replace — kept both). The **combined** tree was re-built green on iPad-lib + desktop and passes `check_core_include_boundaries.sh`. **Runtime-unverified** (build-green only): preset round-trip/apply, `.loredit` parse correctness, XLSX validity, and the IE-15 iPad-app link — all need device/desktop testing.
