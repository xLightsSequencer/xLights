...
|--------|------:|------:|---------|
| **Implemented** | **571** | 51% | At functional parity (often via a touch idiom) |
| **Partial** | **203** | 18% | Present but with real gaps |
| **Missing** | **269** | 24% | Absent but portable |
| **Out-of-scope** | **69** | 6% | Not applicable / not viable on iPad (reason given) |

> **Counts re-tallied as of 2026-06-02** (after the two cross-theme quick-wins batches landed — see the 2026-06-02 rows in [Implementation progress](#implementation-progress)). The **1,112-feature / 23-area baseline** is the 2026-04-23 code-level gap analysis in [`_raw-gap-analysis.md`](_raw-gap-analysis.md), which still sums to 525/223/295/69. Relative to the 2026-06-01 tally (560/206/277/69): **batch 1** moved +9 to implemented (TOOLS-1, FSEQ-1, LIFE-1, TIM-5, TIM-9, LAY-1, VIEW-1, VIEW-3, CTL-27) and **batch 2** moved +2 more from missing (BR-1, LAY-29) → **+11 total** (8 from missing, 3 from partial). Separately, five rows the plans still listed as pending were found **already implemented in the live tree** and corrected to ✅ (CTL-5, COL-2, COL-3, SEQ-19, LAY-37) — not counted in the +11 since they predate these batches. Each `NN-*.md` theme doc's ✅ status callout carries the detail.
...
| **TOOLS-1** | 09/13 | **(2026-06-02)** **Purge Download Cache** — `iPadRenderContext::PurgeDownloadCache()` → `CachedFileDownloader::GetDefaultCache().ClearCache()` + bridge `purgeDownloadCache` + Tools-menu entry (sibling to the landed Purge Render Cache; independent of any open sequence). | ✅ |
| **FSEQ-1** | 09 | **(2026-06-02)** **FSEQ compression preference** — `ReadFseqCompression()`/`ReadFseqCompressionLevel()` read `@AppStorage("fseq.compression")`/`("fseq.compressionLevel")` (CFPreferences) and feed `FSEQFile::createFSEQFile` in `WriteFseq` (was hardcoded `zstd`/2). Picker + zstd-level stepper in **Folder Config → FSEQ Files**. | ✅ |
| **LIFE-1** | 09 | **(2026-06-02)** **Save-As show-folder guard** — after a successful Save-As, a non-blocking warning fires when the destination is outside the show / media folders (`pathIs(inShowOrMediaFolder:)`). The exporter writes the file itself, so this is informational (matches desktop's save-outside-show-dir warning). | ✅ |
| **TIM-5** | 02 | **(2026-06-02)** **Divide Timings by N** — `divideTimingMarks(byN:rowIndex:)` generalizes `halveTimingMarks` (splits each mark into N equal pieces, reverse-boundary order keeps the head index valid). ÷2/÷3/÷4/÷6/÷8 submenu in the timing-row header menu. | ✅ |
| **TIM-9** | 02 | **(2026-06-02)** **Remove Phonemes Only** — bridge `removePhonemesAtRow:` strips just the phoneme layer (2+), keeping phrase + words (lock-guarded). The single "Remove Words / Phonemes" entry becomes a submenu (Remove Words & Phonemes / Remove Phonemes Only) when a phoneme layer exists. | ✅ |
| **LAY-1** | 05/06 | **(2026-06-02)** **DMX model creation** — added the 8 DMX/moving-head tags (`DmxMovingHead`, `…Adv`, `DmxGeneral`, `DmxFloodlight`, `DmxFloodArea`, `DmxServo`, `DmxServo3d`, `DmxSkull`) to `availableModelTypesForCreation` + friendly labels. They create single-point boxed models that the existing tap-to-place flow commits immediately. | ✅ |
| **VIEW-1** | 10 | **(2026-06-02)** **Effect dropper icons** — the `EffectPaletteView` buttons now show each effect's icon (via `EffectIconCache` → `iconBGRA(forEffectNamed:…)`, the same BGRA bitmap the Metal grid uses) ahead of the label. | ✅ |
| **VIEW-3** | 10 | **(2026-06-02)** **Dock All + Reset Pane Sizes** — View-menu `XLDockAllCommands`: "Dock All Windows" dismisses every detached preview/inspector scene in one action; "Reset Pane Sizes" clears `previewPaneHeight`/`inspectorPaneWidth` so the `@AppStorage` bindings re-lay-out at defaults. | ✅ |
| **CTL-27** | 07 | **(2026-06-02)** **FPP Connect final-status parity** — the done view now surfaces desktop's status-bar string ("FPP Connect Upload Complete" / "…had errors or warnings" / "…Cancelled"). Intermediate per-sequence "Preparing/Checking" labels remain a follow-up. | ✅ (final-status slice) |
| **CTL-5 / COL-2 / COL-3 / SEQ-19 / LAY-37** | 07/04/01/05 | **(correction, 2026-06-02)** Found **already implemented** in the live tree, plans were stale: CTL-5 (Add-FPP-by-IP + Re-Discover), COL-2 (VC bulk-apply to selection), COL-3 (palette Reverse/Shift), SEQ-19 (Select All Marks → `selectAllEffectsInRow`), LAY-37 (model lock toggle + `"locked"` bridge property). Status callouts flipped to ✅; not counted in the +9. | ✅ (pre-existing) |
| **BR-1** | 09 | **(2026-06-02, batch 2)** **Batch-render scope toggle** — `SequenceScanner.scan(showFolder:recursiveSubfolders:)` gained a scope arg (false = show folder only, no descent into subfolders); an "Include subfolders" Toggle (`@AppStorage("batchRender.includeSubfolders")`, default on) in `BatchRenderSheet` re-scans on change. | ✅ |
| **LAY-29** | 05/06 | **(2026-06-02, batch 2)** **Generate Node Names (DMX)** — bridge `generateNodeNamesForModel:` → `DmxModel::GenerateNodeNames()` (guarded on `IsDMXModel()`); a "Generate" toolbar button in the node-names editor (`IndexedNamesEditorSheet`) fills the editable rows for DMX models so the user can review/edit before Save. | ✅ |
...
> **Cross-theme quick-wins batch 1 (2026-06-02):** nine small, file-disjoint parity items landed in one session, spanning **9 themes** to avoid concentrating on any one area (the nine TOOLS-1/FSEQ-1/LIFE-1/TIM-5/TIM-9/LAY-1/VIEW-1/VIEW-3/CTL-27 rows above). Research was fanned out across read-only Explore agents; edits were applied serially in the main tree to keep the hub files (`SequencerViewModel`, `XLSequenceDocument`, `XLightsCommands`) conflict-free, with batched `xLights-iPadLib` Debug builds. **All build-verified green (`xLights-iPadLib`, Debug, arm64).** Five further items (CTL-5, COL-2, COL-3, SEQ-19, LAY-37) were found already implemented and only needed status corrections. **Runtime-unverified** (build-green only): the FSEQ compression round-trip, Divide/Remove-Phonemes timing edits, DMX model placement, and the dropper-icon render all need on-device confirmation. **Verify-against-current-code paid off again** — half the originally-scoped wins were already done.

> **Cross-theme quick-wins batch 2 (2026-06-02):** a follow-on pass landed **BR-1** (batch-render subfolder scope) and **LAY-29** (DMX Generate Node Names), both build-verified + boundary-check clean. The notable signal: **the easy-win seam is thinning.** Of seven batch-2 candidates, five were deferred after verify-first research because they aren't actually clean S items: **FX-15** (Pictures decode warning — a UIImage check risks false positives vs the core decode path), **RENDER-2** (time-slice render — needs waveform-selection UI + a `RenderRange` core entry), **VIEW-7** (Select-Effects filter — needs a new effect-enumeration bridge surface; `EffectInfo` only carries name+time), **LAY-26** (preview rename/delete — no core LayoutGroup delete/rename; deleting must reassign every referencing model + handle reserved groups), **COL-8** (named swatches — ambiguous target slot, largely redundant with the native iOS `ColorPicker`), **MED-6** (AI-into-media-manager — wrong surface: the manager lists *referenced* media, so an unassigned generated image wouldn't appear). Remaining parity work increasingly needs new bridge surface or core changes rather than pure SwiftUI wiring — the next pass should plan around the keystones (node-select grid, automation dispatcher, effect-enumeration bridge) rather than hunting for more standalone S items.
