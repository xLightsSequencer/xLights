...
| Feature | Theme | Ease | Feasibility | Notes |
|---|---|---|---|---|
| **Effect Symbols: link / unlink / convert UI** | 04 | medium | feasible | Investigated and part-built 2026-07-25 (see theme 04). Data safety is **resolved**: storage, save/load and edit-propagation are shared core, a paste/undo-of-delete/move bug that dropped the link is fixed, and the grid now draws desktop's cyan corner triangle so a linked effect is visible before you edit it. What remains is management: no Create Symbol / Unlink / Link-to / Convert-All on iPad, so links can be seen but not authored or broken. |
