...
**Vetted backlog (feasibility-spiked this round — ready for a dedicated effort, NOT done):**
- **IE-4 / IE-5 legacy LOR import** — *feasible-with-extraction (L).* **Scope correction:** the plan's `ReadLorFile` reference was wrong — that's the channel-data converter; the real effect-level importers are `ImportLMS`/`ImportS5`/`LOREdit` (in `ImportEffects.cpp` / `LOREdit.cpp`), whose parse is overwhelmingly pugixml + `std::string` + `xlColor` (shallow wx surface, no zip). Extract `LOREdit` first (lowest entanglement) into `src-core/import_export`, rewire desktop to share it (parity), then wire `XLImportSession` to consume it like `EffectMapper`. Runtime-unverifiable (parse correctness) → needs a careful dedicated PR.
- **PRE-1 persistent preset library** — *feasible (L, keystone).* Relocate `EffectPresetManager` to wx-free core; persist to `<showFolder>/xlights_effectpresets.json` (desktop format → cross-platform round-trip); desktop keeps `EffectTreeDialog` UI + its apply path, iPad adds a browser + bridge (`presetTree`/`savePreset`/`applyPreset`/…). Unlocks the doc-10 preset chain.
- **IE-15 Export Models XLSX** — *feasible but currently BLOCKED on a link flag.* `libxlsxwriter.a` is present for all iOS slices but the `xLights-iPad` app target's `OTHER_LDFLAGS` omits `-lxlsxwriter` (and `$(inherited)`); add it (pbxproj) first, then a wx-free core `ExportModels` (data is all core-resident; only cosmetic wx) + bridge + `ShareLink`.
