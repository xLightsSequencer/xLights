...
| Sequences ▸ Default Duration for new sequences | preference | ✅ | ❌ | desktop-missing | P3 | easy | feasible | Desktop: "Set Default" button in `SeqSettingsDialog` Info/Media tab writes `DefaultSeqDuration` (string, e.g. `"30.000"`) via `GetXLightsConfig()`; applied to `SequenceFile` before the new-sequence wizard opens (`SeqFileUtilities.cpp`). iPad: `SequenceSettingsSheet` (or `FolderConfigView`) should expose a duration picker that writes `@AppStorage("sequence.defaultDuration")` (default `30.0`); `newSequenceAtPath:` reads it and passes to `SequenceFile::SetSequenceDuration` before save. |
| Sequences ▸ Default Timing for new sequences | preference | ✅ | ❌ | desktop-missing | P3 | easy | feasible | Desktop: "Set Default" button in `SeqSettingsDialog` writes `DefaultSeqTiming` (string, e.g. `"25 ms"`) via `GetXLightsConfig()`; applied to `SequenceFile` before the new-sequence wizard opens (`SeqFileUtilities.cpp`). iPad new-sequence sheet already requires timing selection — wire `@AppStorage("sequence.defaultTiming")` (default `"25 ms"`) to pre-select the picker and write it to `SequenceFile::SetSequenceTiming` in `newSequenceAtPath:`. |
...
| Add Timing Mark (T) | shortcut | ✅ | 🟡 | ipad-missing | P3 | medium | feasible | Desktop TIMING_ADD=t `KeyBindings.cpp:345`. iPad has `addTimingMark(rowIndex:startMS:endMS:)` but only via long-press, which supplies the target row + span from touch context. A key binding has to resolve the target itself — active timing track (`document.timingRowIsActive(at:)`) plus the cell under `playPositionMS` — and match desktop's split-the-current-cell semantics. Deliberately held back from the L/U/⇧I/⇧A/⇧X shortcut pass: not verifiable without a device, and wrong semantics here silently damage timing tracks. |
| Split Timing Mark (S) | shortcut | ✅ | 🟡 | ipad-missing | P3 | medium | feasible | Desktop TIMING_SPLIT=s `KeyBindings.cpp:346`. Same blocker as Add Timing Mark: `splitTimingMark(rowIndex:markIndex:atMS:)` needs the mark under the playhead on the active track resolved first. Held back from the same pass. |
| Insert Layer Above/Below (Shift+I / Shift+A) | shortcut | ✅ | ✅ | parity | P3 | easy | feasible | Desktop `KeyBindings.cpp:371`. iPad: Edit menu "Insert Layer Above/Below" (`XLightsCommands.swift`) on ⇧I / ⇧A → `SequencerViewModel.insertLayerRelativeToSelection(above:)`, acting on the selected effect's row (same `insertEffectLayerAbove/Below` bridge calls as the row-header context menu). |
| Toggle Element Expand (Shift+X) | shortcut | ✅ | ✅ | parity | P3 | easy | feasible | Desktop `KeyBindings.cpp:373`. iPad: Edit menu "Toggle Element Expand" on ⇧X → `toggleElementExpandForSelection()` → `toggleElementCollapsed(at:)` on the selected effect's row (the tap path's bridge call). |
| Lock (L) / Unlock (U) effect | shortcut | ✅ | ✅ | parity | P3 | easy | feasible | Desktop `KeyBindings.cpp:361`. iPad: Edit menu "Lock Effect" / "Unlock Effect" on L / U → `setLockOnSelectedEffects(_:)`, which sets an explicit state (not a toggle) across the whole selection so a mixed multi-select lands all one way, in a single undo group. |
...
| Default sequence duration + frame rate for new sequences | preference | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop #5233 adds a stored default duration and fps applied to new sequences (`SeqSettingsDialog.cpp` / `SeqFileUtilities.cpp`). iPad's New-Sequence wizard always starts from its built-in duration + a 25/50/100 fps picker; `FolderConfigView` has the sibling `sequence.defaultModelBlending` pref but no duration/fps defaults. Work: two more `@AppStorage` keys read by the wizard — the same shape as the blending pref already wired through `newSequenceAtPath:`. |
| Preferences ▸ Toolbars page (choose + reorder Effects toolbar entries) | preference | ✅ | ➖ | ipad-missing | P3 | medium | infeasible-idiom | Desktop's new `ToolbarsSettingsPanel` (`src-ui-wx/preferences/ToolbarsSettingsPanel.cpp`) picks which effects appear in the always-visible Effects toolbar and their order, with a right-click shortcut from the toolbar itself. The iPad has no always-visible effects toolbar to configure — effects come from the effect library sheet and the command palette — so there is no toolbar to curate. Revisit only if the iPad grows a pinned effect bar. |
| "API Port" naming + visible port numbers | preference | ✅ | ❌ | ipad-missing | P3 | easy | feasible | Desktop renamed the "Automation" port to "API" port and now shows the port numbers (`src-ui-wx/preferences/OutputSettingsPanel.cpp:55`). The iPad exposes no API/automation port setting at all (see theme 12 for the automation surface), so this is a naming change on a preference the iPad lacks — recorded for completeness; it becomes relevant only if the iPad ever hosts the API listener. |
| Key bindings stored in show folder + app-data, with a "Use keybindings from" choice | preference | ✅ | ➖ | ipad-missing | P3 | medium | infeasible-idiom | Desktop keeps key bindings in both the show folder and the platform app-data folder, syncs them on every save/backup/restore, and adds Preferences ▸ Other ▸ "Use keybindings from" to pick which copy wins (`src-ui-wx/preferences/OtherSettingsPanel.cpp:180`). The iPad's shortcuts are hardcoded in `XLightsCommands.swift` with a read-only `KeyBindingsSheet`, and this theme already records user-customizable keybindings as a large, low-value iPad item — the dual-storage question only arises once bindings are editable. |
| Preferences dialog uses a left-hand icon list instead of top tabs (Windows/Linux) | dialog | ✅ | ➖ | n/a | P3 | — | infeasible-idiom | Presentation change to the desktop preferences dialog on Windows/Linux. The iPad's settings live in `FolderConfigView` / sheets with their own navigation; nothing to mirror. |
