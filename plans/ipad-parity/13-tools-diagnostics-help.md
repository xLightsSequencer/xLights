...
|---|---|---|---|---|---|---|---|---|
| Tools → Test (Pixel/Light Test) | menu | ✅ | 🟡 | ipad-weaker | P1 | hard | feasible | **Landed (Models + Controllers).** The old "iOS sandbox blocks raw output" verdict was stale — the app ships `com.apple.developer.networking.multicast` (`xLights-iPad.entitlements:9`) plus `NSLocalNetworkUsageDescription`, and already drives live output during playback (`XLSequenceDocument.mm` `startOutput` → `OutputManager::StartOutput`). Pattern maths extracted from `PixelTestDialog::OnTimer` into wx-free core `src-core/outputs/TestPatternEngine.{h,cpp}` + `ChannelTracker.{h,cpp}`; desktop now calls the same engine. iPad: `Bridge/XLLightTest.{h,mm}` → `App/LightTestSheet.swift`, Tools → "Test Lights…". Test presets round-trip through the show's networks file, so desktop-saved selections load on iPad and vice versa. **Remaining gap:** the Outputs / Model Groups tabs and the visual tap-a-node-on-the-preview Model tab; serial/USB controllers are listed as untestable (no iPadOS transport). |
...
| Tools → FPP Connect | menu | ✅ | ✅ | parity | P1 | easy | feasible | `xLightsMain.cpp:1113` / `FPPConnectSheet.swift` (1314), `FPPConnectMenuItem`. Discover FPP + upload `.fseq`. Open-firmware path. 2026-07-16: both sides read the source `.fseq` with `FSEQFile::ReadPattern::Bulk` (parallel block decompress in shared `src-core/render/FSEQFile.cpp`) when building an upload — desktop `FPPConnectDialog.cpp`, iPad `XLSequenceDocument.mm`. Same core, matching opt-in on both. |
...
| Startup machine-config banner | log | ✅ | ✅ | parity | P2 | easy | feasible | Desktop `DumpConfig()` (`xLightsApp.cpp:358`) / iPad `LogMachineConfig()` (`XLiPadInit.mm:149`). Both now log CPU brand, physical/logical core counts and the Metal GPU via the shared wx-free `GetCPUBrand` / `GetPhysicalCoreCount` / `GetLogicalCoreCount` / `GetGPUDescription` in `src-core/utils/UtilFunctions.{h,cpp}` (`GetGPUDescription` implemented for Apple in `src-core/effects/metal/MetalComputeUtilities.mm`, no-op elsewhere). Added so crash reports carry the hardware needed to reproduce threading and GPU issues. iPad's banner additionally carries device model, thermal state, free disk and locale. |
| Machine config attached to crash report | crash zip | ✅ | ✅ | parity | P2 | easy | feasible | The log is not a reliable carrier — it rotates at 20MB, and measuring the archive showed ~6% of desktop reports arriving with no machine record at all, with the rolled `.1.log` present in only 1 report in 2,500. Desktop now attaches the banner verbatim as `machine_config.txt` via `wxDebugReport::AddText` in `AddDebugFilesToReport` (`xLightsMain.cpp`), fed by `GetMachineConfigSummary()` (`xLightsApp.h`). iPad already wrote an unconditional `device-info.txt` (`XLLogPackager.mm:62` `DeviceInfoText()`); it now carries the same CPU brand / physical-core / GPU fields. |
...
| Help → About | menu | ✅ | ✅ | parity | P1 | easy | feasible | `xLightsMain.cpp:1296`, `OnAbout`; iPad `AboutSheet.swift` / `showingAbout`. 2026-07-31: the name is now marked “xLights™” on both sides in the same change — desktop About heading (`xLightsMain.cpp:2674`) and app-name label (`AboutDialog.cpp`), iPad `AboutSheet.swift:38,63` plus the first-run show-folder setup screen (`XLightsApp.swift:999`). Deliberately **not** in the window title bar / iPad scene title: no comparable app marks there, and those carry the document name. The “xLights is a trademark of xLights LLC.” ownership line lives in the shared `XLIGHTS_LICENSE` (`include/globals.h`), so it renders in both About boxes automatically. |
| Diagnostics: spdlog rotation | other | ✅ | ✅ | parity | P1 | easy | feasible | Same rotating-file config in core + `XLiPadInit.mm`. |
| Diagnostics: crash capture + auto-upload | other | ✅ | ✅ | parity | P1 | easy | feasible | Desktop uploads to dankulp.com/crashUpload; iPad `XLDiagnosticUploader.swift` + MetricKit auto-stages & POSTs to the same endpoint, with Settings opt-in (`XLSendCrashReports`). iPad slightly ahead (automatic MetricKit). 2026-07-29: every fixed-format filename stamp on both platforms now pins `en_US_POSIX` (`XLMetricKit.mm`, `XLLogPackager.mm`, `XLDiagnosticUploader.swift`, `SequencerViewModel.swift` save-backup) — without it a 12-hour region rewrote `HH` to `hh` plus a localised AM/PM marker, putting non-ASCII bytes in the payload/upload/backup name, and the resulting MetricKit payloads could not be extracted from the report zip at all. Fixed on both sides in the same change. |
...
| Per-show id for crash-report de-duplication | diagnostics | ✅ | ✅ | parity | P2 | easy | feasible | 2026-07-29. Crash reports carried nothing that could group them, so one show crashing repeatedly counted N times in the show statistics. Both clients now mint `<settings><ShowGUID>` in `xlights_rgbeffects.xml` on first open if absent — random v4 UUID from the shared wx-free `GenerateGuid()` (`src-core/utils/string_utils.cpp`), identifying the show only. Desktop sets it via `SetXmlSetting` during `LoadEffectsFile` (`TabSequence.cpp`) so the existing dirty/autosave path persists it, and it reaches the server inside the already-attached `xlights_rgbeffects.xml`. iPad writes it straight back in `iPadRenderContext::LoadShowFolder` (deferring would mean a different id every launch) and reports it as `showguid` in the counts-only `xlShowStats.txt` sidecar, since the automatic iPad upload deliberately carries no show XML. Because the id travels with the show folder it also ties one show together across Mac, Windows and iPad. |
