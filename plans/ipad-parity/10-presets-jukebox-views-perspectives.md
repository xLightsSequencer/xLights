...
| **Display Elements: panel-local Undo** | menu | ✅ | ✅ | parity | P2 | medium | feasible | Desktop `ViewsModelsPanel.cpp:1664` (`ID_MODELS_UNDO`) keeps a *panel-scoped* stack — `SaveUndo()` pushes the selected view's model column before every membership/order change, `Undo()` replays it through `SetMasterViewModels` / `SequenceView::SetModels` + `AddViewToTimings`, and `ClearUndo()` drops it on view change. **Landed 2026-07-25.** iPad `DisplayElementsSheet` now has the same thing: an **Undo** toolbar button over a local `[PanelSnapshot]` stack (deliberately *not* the sequencer's global undo manager, matching desktop's scoping), pushed by every membership / order / bulk action — add + add-all, remove + remove-all, Remove Unused, the 8-way Sort, and drag-reorder — and cleared on `selectedViewIdx` change. The snapshot additionally carries per-element visibility, so **Show All / Hide All / Hide Unused** are recoverable too; desktop's stack holds membership only. Master-view semantics match desktop exactly, including its limitation: undoing a *removal* re-creates the element but not the effects that went with it (hence the warning on the remove path), and undoing an *add* is a no-op because `SetMasterViewModels` never removes extras. User views are a clean full replace — they only reference model names, so nothing can be lost. |
...
| Presets track unsaved changes independently of the RGB effects file | core | ✅ | 🟡 | ipad-weaker | P3 | medium | feasible | Desktop now tracks preset dirtiness separately, so preset-only edits no longer mark the RGB effects file dirty. iPad has no equivalent split (`src-iPad/` has no preset-dirty state), so a preset edit still rides on the general layout/effects dirty flag. Mostly a save-prompt-noise issue rather than data loss. |
| Effect Presets window dockable into the main window | panel | ✅ | ➖ | ipad-missing | P3 | — | infeasible-idiom | Desktop #5047 lets the Effect Presets window dock via wxAUI instead of floating. iPad presets live in `PresetBrowserSheet` (a sheet); iPadOS has no AUI-style dock-a-panel idiom — the existing detached-window/Stage-Manager split is the closest analogue. Recorded as idiom-infeasible, consistent with the other AUI dock rows in this theme. |
| Smart Presets: relative + auto layer-mode presets, paste auto-inserts layers | preset | ✅ | 🟡 | ipad-weaker | P2 | medium | feasible | Desktop #6428 adds relative and auto layer-mode presets plus a Ctrl/Cmd+V paste that auto-inserts the layers a multi-layer preset needs. The iPad **already auto-inserts layers on paste** (`SequencerViewModel.pasteAtRow`, noted in theme 02), but has no relative / auto layer-mode *preset* concept — the mode is what is missing, not the paste behaviour. |
| Access base-show-folder presets | panel | ✅ | ✅ | parity | P2 | medium | feasible | Desktop #2524 exposes presets stored in the base show folder. iPad already had this as the read-only "From Base" section (row above) — parity by convergence. |
...
- **Display Elements panel-local Undo** — *landed.* Local
  `[PanelSnapshot]` stack + Undo toolbar button in `DisplayElementsSheet`,
  mirroring desktop's panel-scoped `_undo` and additionally covering the
  bulk visibility actions. See the scorecard row.
