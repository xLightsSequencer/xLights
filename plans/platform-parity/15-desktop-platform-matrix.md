...
| Video decode | AVFoundation (HW on by default), FFmpeg fallback | FFmpeg (HW off by default) + optional Media Foundation | FFmpeg, HW pref hidden | Video effects render faster and frame-parallel on macOS; Win/Linux always take the stateful path. |
...
| 36 | Video decode | "Hardware Video Decoding" preference means different things and defaults differently | **on**; doubles as the AVFoundation↔FFmpeg switch | off; enables cuda/qsv/d3d11va/vulkan or the Media Foundation reader | **checkbox hidden**, off | `VideoReader.cpp:62`, `:81`; defaults `src-ui-wx/xLightsMain.cpp:2109` vs `:2123-2126`; hidden `src-ui-wx/preferences/OtherSettingsPanel.cpp:266-270` |
...
| 39 | Video decode | `WindowsHardwareVideoReader` (Media Foundation) — Windows-only, and gated on the pref **and** renderer == DirectX11 while the renderer defaults to FFmpeg Auto, so it is off out of the box | not compiled | compiled, off by default | not compiled | `FFmpegVideoReader.cpp:163-190`; `src-core/media/WindowsHardwareVideoReader.cpp:390`, `:422`; defaults `xLightsMain.cpp:2124` |
