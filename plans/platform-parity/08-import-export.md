...
| 54 | House-preview export — output resolution choice | `HousePreviewExportOptionsDialog` (`src-ui-wx/layout/HousePreviewExportOptionsDialog.h/.cpp`) — 720p / 1080p / 1440p / 4K / Current-size / Custom picker shown from `OnMenuItem_File_Export_VideoSelected` (`xLightsMain.cpp:3694-3714`); `ExportVideoPreview` (`:3718`) renders at the chosen size by temporarily resizing the House Preview canvas (`:3760-3770`) rather than an offscreen buffer. Also exposed to automation via `exportVideoPreview`'s `width`/`height` params (`xLightsAutomations.cpp:856-858`) | ✅ | 720p / 1080p / 4K / Match-preview picker (`ExportHousePreviewSheet.swift:24-53`), rendered offscreen independent of the pane (`XLSequenceDocument.h:683-686`). **Minor gap:** no 1440p/custom preset and no scripting equivalent (row 82) |
