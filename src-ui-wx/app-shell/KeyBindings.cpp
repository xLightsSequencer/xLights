...
static  std::vector<std::pair<std::string, KBSCOPE>> KeyBindingTypes =
{
    ...
    { "COPY_MODEL_LAYERS_TO_MODELS", KBSCOPE::Sequence },
...
static  std::vector<std::pair<std::string, std::string>> keyBindingTips = {
    { "TIMING_ADD", "Add a timing mark at the playback cursor on the active timing track." },
    { "TIMING_SPLIT", "Split the timing mark under the cursor into two at the cursor position." },
    { "ZOOM_IN", "Zoom the effects grid in for finer timing detail." },
    { "ZOOM_OUT", "Zoom the effects grid out to see more of the timeline." },
    { "ZOOM_SEL", "Zoom the timeline so the currently selected time range fills the grid." },
    { "RANDOM", "Insert randomly chosen effects into the selected cells." },
    { "RENDER_ALL", "Render every model in the sequence from scratch." },
    { "SAVE_CURRENT_TAB", "Save whichever tab is currently active (sequence, layout, or setup)." },
    { "LIGHTS_TOGGLE", "Turn live output to your controllers/lights on or off." },
    { "OPEN_SEQUENCE", "Open an existing sequence from the show folder." },
    { "CLOSE_SEQUENCE", "Close the currently open sequence (prompts to save if changed)." },
    { "NEW_SEQUENCE", "Create a new, empty sequence." },
    { "PASTE_BY_CELL", "Switch paste mode so copied effects align to timing cells rather than exact times." },
    { "PASTE_BY_TIME", "Switch paste mode so copied effects keep their original absolute times." },
    { "BACKUP", "Back up the show folder to the configured backup location." },
    { "ALTERNATE_BACKUP", "Back up the show folder to the alternate (secondary) backup location." },
    { "SELECT_SHOW_FOLDER", "Switch to a different show folder." },
    { "SAVEAS_SEQUENCE", "Save the current sequence under a new file name." },
    { "SAVE_SEQUENCE", "Save the current sequence." },
    { "EFFECT_SETTINGS_TOGGLE", "Show or hide the Effect Settings panel where the selected effect's options are edited." },
    { "EFFECT_ASSIST_TOGGLE", "Show or hide the Effect Assist panel (visual editor for effects that support it)." },
    { "COLOR_TOGGLE", "Show or hide the Colors panel used to set an effect's palette." },
    { "LAYER_SETTING_TOGGLE", "Show or hide the Layer Settings panel (buffer style, transform, blur, etc.)." },
    { "LAYER_BLENDING_TOGGLE", "Show or hide the Layer Blending panel that controls how stacked layers combine." },
    { "MODEL_PREVIEW_TOGGLE", "Show or hide the per-model preview panel." },
    { "HOUSE_PREVIEW_TOGGLE", "Show or hide the whole-house preview panel." },
    { "EFFECTS_TOGGLE", "Show or hide the Effects panel you drag effects from onto the grid." },
    { "DISPLAY_ELEMENTS_TOGGLE", "Show or hide the Display Elements panel for choosing which models/views appear in the grid." },
    { "JUKEBOX_TOGGLE", "Show or hide the Jukebox panel of saved effect buttons." },
    { "SEQUENCE_SETTINGS", "Open the Sequence Settings dialog (timing, media, metadata)." },
    { "LOCK_EFFECT", "Lock the selected effects so they can't be moved or edited." },
    { "UNLOCK_EFFECT", "Unlock the selected effects so they can be moved or edited again." },
    { "MARK_SPOT", "Remember the current timeline position so you can jump back to it later." },
    { "RETURN_TO_SPOT", "Jump back to the timeline position you previously marked." },
    { "EFFECT_DESCRIPTION", "Add or edit a text note/description on the selected effect." },
    { "EFFECT_ALIGN_START", "Move the selected effects so they all share the first one's start time." },
    { "EFFECT_ALIGN_END", "Move the selected effects so they all share the first one's end time." },
    { "EFFECT_ALIGN_BOTH", "Stretch the selected effects so they all share the first one's start and end times." },
    { "INSERT_LAYER_ABOVE", "Add a new, empty effect layer above the current model row." },
    { "INSERT_LAYER_BELOW", "Add a new, empty effect layer below the current model row." },
    { "COPY_MODEL_LAYERS_TO_MODELS", "Copy the selected model's layers/submodels onto one or more other models (you choose the targets)." },
    { "TOGGLE_ELEMENT_EXPAND", "Expand or collapse the selected model row to show/hide its strands, submodels and nodes." },
    { "SELECT_ALL", "Select every effect and timing mark on the current row." },
    { "SELECT_ALL_NO_TIMING", "Select every effect on the current row, leaving timing marks unselected." },
    { "SHOW_PRESETS", "Open the Effect Presets panel of saved effect definitions." },
    { "SEARCH_TOGGLE", "Show or hide the effect search panel." },
    { "FILTER_SEQUENCER", "Jump to the sequencer's prop filter box to type a model name and narrow the visible rows." },
    { "PERSPECTIVES_TOGGLE", "Show or hide the Perspectives panel for saving/restoring panel layouts." },
    { "EFFECT_UPDATE", "Copy the current effect's settings onto all other selected effects." },
    { "COLOR_UPDATE", "Copy the current effect's colors onto all other selected effects." },
    { "PLAY_LOOP", "Play the selected time range over and over." },
    { "PLAY", "Start playback from the playback cursor." },
    { "TOGGLE_PLAY", "Start playback, or stop it if already playing." },
    { "START_OF_SONG", "Move the playback cursor to the start of the sequence." },
    { "END_OF_SONG", "Move the playback cursor to the end of the sequence." },
    { "STOP", "Stop playback and return output to idle." },
    { "PAUSE", "Pause playback, keeping the cursor where it is." },
    { "EFFECT", "Drop a specific effect (configured on this binding) into the selected cells." },
    { "APPLYSETTING", "Apply a saved setting (configured on this binding) to the selected effects." },
    { "PRESET", "Drop a saved preset effect (configured on this binding) into the selected cells." },
    { "LOCK_MODEL", "Lock the selected models so they can't be moved on the layout." },
    { "UNLOCK_MODEL", "Unlock the selected models so they can be moved again." },
    { "GROUP_MODELS", "Create a new model group containing the selected models." },
    { "WIRING_VIEW", "Open the wiring view showing node order and connections for the selected model." },
    { "EXPORT_MODEL_CAD", "Export the selected model as a CAD file (DXF, STL, or VRML)." },
    { "EXPORT_LAYOUT_DXF", "Export the entire default layout as a DXF file." },
    { "NODE_LAYOUT", "Open the node layout view showing each node's position for the selected model." },
    { "SAVE_LAYOUT", "Save changes made on the Layout tab." },
    { "SELECT_ALL_MODELS", "Select every model on the layout." },
    { "MODEL_ALIGN_TOP", "Align the selected models so their top edges match the first selected model." },
    { "MODEL_ALIGN_BOTTOM", "Align the selected models so their bottom edges match the first selected model." },
    { "MODEL_ALIGN_LEFT", "Align the selected models so their left edges match the first selected model." },
    { "MODEL_ALIGN_RIGHT", "Align the selected models so their right edges match the first selected model." },
    { "MODEL_ALIGN_CENTER_VERT", "Align the selected models to share the same vertical center line." },
    { "MODEL_ALIGN_CENTER_HORIZ", "Align the selected models to share the same horizontal center line." },
    { "MODEL_ALIGN_FRONTS", "Align the selected models' front (near) edges in 3D." },
    { "MODEL_ALIGN_BACKS", "Align the selected models' back (far) edges in 3D." },
    { "MODEL_ALIGN_GROUND", "Drop the selected models so they sit on the ground plane in 3D." },
    { "MODEL_DISTRIBUTE_HORIZ", "Space the selected models evenly left-to-right." },
    { "MODEL_DISTRIBUTE_VERT", "Space the selected models evenly top-to-bottom." },
    { "MODEL_FLIP_HORIZ", "Mirror the selected models left-to-right." },
    { "MODEL_FLIP_VERT", "Mirror the selected models top-to-bottom." },
    ...
    { "AUDIO_F_1_5_SPEED", "Play audio back at 1.5x speed." },
    { "AUDIO_F_2_SPEED", "Play audio back at 2x speed." },
    { "AUDIO_F_3_SPEED", "Play audio back at 3x speed." },
    { "AUDIO_F_4_SPEED", "Play audio back at 4x speed." },
    { "AUDIO_S_3_4_SPEED", "Play audio back at 3/4 (0.75x) speed." },
    { "AUDIO_S_1_2_SPEED", "Play audio back at 1/2 (0.5x) speed." },
    { "AUDIO_S_1_4_SPEED", "Play audio back at 1/4 (0.25x) speed." },
    { "PRIOR_TAG", "Move the cursor to the previous audio tag." },
    { "NEXT_TAG", "Move the cursor to the next audio tag." },
    { "PLAY_PRIOR_TAG", "Start playback from the previous audio tag." },
    { "PLAY_NEXT_TAG", "Start playback from the next audio tag." },
    { "MODEL_SUBMODELS", "Open the submodels editor for the selected model." },
    { "MODEL_FACES", "Open the faces editor for the selected model." },
    { "MODEL_STATES", "Open the states editor for the selected model." },
    { "MODEL_MODELDATA", "Open the custom-model data grid for the selected model." },
    { "MODEL_TOGGLE", "Enable or disable rendering of the selected model in the sequence." },
    { "MODEL_DISABLE", "Disable rendering of the selected model so it produces no output." },
    { "MODEL_ENABLE", "Re-enable rendering of the selected model." },
    { "EFFECT_TOGGLE", "Enable or disable rendering of the selected effects." },
    { "EFFECT_DISABLE", "Disable the selected effects so they produce no output." },
    { "EFFECT_ENABLE", "Re-enable the selected effects." },
    { "MODEL_EFFECT_TOGGLE", "Enable/disable rendering of the selected effects, or the whole model if none are selected." },
    { "EFFECTS_TO_TIMING", "Create timing marks at the start/end of each selected effect." },
    { "SELECT_TIMING_1", "Make the 1st timing track the active one." },
    { "SELECT_TIMING_2", "Make the 2nd timing track the active one." },
    { "SELECT_TIMING_3", "Make the 3rd timing track the active one." },
    { "SELECT_TIMING_4", "Make the 4th timing track the active one." },
    { "SELECT_TIMING_5", "Make the 5th timing track the active one." },
    { "SELECT_TIMING_6", "Make the 6th timing track the active one." },
    { "SELECT_TIMING_7", "Make the 7th timing track the active one." },
    { "SELECT_TIMING_8", "Make the 8th timing track the active one." },
    { "SELECT_TIMING_9", "Make the 9th timing track the active one." },
    { "SELECT_NO_TIMING", "Deselect the active timing track." },
    { "INCREASE_SPEED", "Increase the playback speed." },
    { "DECREASE_SPEED", "Decrease the playback speed." },
    { "JUKEBOX_BTN_1", "Trigger Jukebox button 1." },
    { "JUKEBOX_BTN_2", "Trigger Jukebox button 2." },
    { "JUKEBOX_BTN_3", "Trigger Jukebox button 3." },
    { "JUKEBOX_BTN_4", "Trigger Jukebox button 4." },
    { "JUKEBOX_BTN_5", "Trigger Jukebox button 5." },
    { "FPP_CONNECT", "Open FPP Connect to upload sequences/config to your players." },
    { "COMMAND_PALETTE", "Open the command palette to search and run any command." },
    { "IMPORT_EFFECTS", "Open the Import Effects dialog to bring effects in from another sequence." },
