...
void ModelFacesPanel::OnDeactivate()
{
    ...
    // Don't leave this tab's timer driving the real controller output once
    // it's no longer visible - it would fight with another tab's output.
    StopOutputToLights();
    CheckBox_OutputToLights->SetValue(false);
}
...
void ModelFacesPanel::ApplyStateOutlineOverlay()
{
    ...
    auto stateInfoMap = _getStateInfo ? _getStateInfo() : model->GetStateInfo();
    ...
    auto stateNodes = Model::ComputeStateInfoNodes(stateInfoMap);
...
std::vector<int> ModelFacesPanel::GetDragSelectedNodes(bool freeform)
{
    if (freeform) {
...
void ModelFacesPanel::SelectAllInBoundingRect(bool shiftDwn, bool freeform)
{
    if (shiftDwn) {
...
void ModelFacesPanel::RemoveNodes(bool freeform)
{
    const std::string name = NameChoice->GetString(NameChoice->GetSelection()).ToStdString();
    if (name == "") {
        return;
