...
public:
    CtrlAttrProxyModel(wxDataViewModel* inner, OutputManager* om, int statusCol) :
        _inner(inner), _om(om), _statusCol(statusCol) {
        _inner->IncRef();
    ...
    bool GetAttr(const wxDataViewItem& item, unsigned int col, wxDataViewItemAttr& attr) const override {
        Controller* c = _om->GetController(DataViewTopLevelName(_inner, item));
        ...
        if ((int)col == _statusCol && c->IsActive()) {
            // colour the ● status dot: green online, red offline, grey unknown
            auto ps = c->GetLastPingState();
            wxColour dot(210, 40, 40);
            if (ps == Output::PINGSTATE::PING_OK || ps == Output::PINGSTATE::PING_WEBOK) {
                dot = wxColour(40, 180, 40);
            } else if (ps == Output::PINGSTATE::PING_UNKNOWN || ps == Output::PINGSTATE::PING_UNAVAILABLE) {
                dot = wxColour(128, 128, 128);
            }
            attr.SetColour(dot);
            return true;
        }
...
private:
    ...
    public:
        ...
        bool ItemDeleted(const wxDataViewItem& parent, const wxDataViewItem& item) override { return _t->ItemDeleted(parent, item); }
        bool ItemChanged(const wxDataViewItem& item) override { return _t->ItemChanged(item); }
        bool ItemsAdded(const wxDataViewItem& parent, const wxDataViewItemArray& items) override { return _t->ItemsAdded(parent, items); }
        bool ItemsDeleted(const wxDataViewItem& parent, const wxDataViewItemArray& items) override { return _t->ItemsDeleted(parent, items); }
        bool ItemsChanged(const wxDataViewItemArray& items) override { return _t->ItemsChanged(items); }
        bool ValueChanged(const wxDataViewItem& item, unsigned int col) override { return _t->ValueChanged(item, col); }
        bool Cleared() override { return _t->Cleared(); }
        bool BeforeReset() override { return _t->BeforeReset(); }
        bool AfterReset() override { return _t->AfterReset(); }
...
void ControllerListPanel::CreateTree() {
...

    {
        ...
        auto* proxy = new CtrlAttrProxyModel(dv->GetModel(), _frame->GetOutputManager(), -1);
        dv->AssociateModel(proxy);
        proxy->DecRef(); // the view holds the remaining reference
        _ctrlAttrModel = proxy;
    }
...
void ControllerListPanel::OnBeginDrag(wxDataViewEvent& event) {
    ...
    wxString payload = "xlctrlmove:";
    for (const auto& n : names) {
        if (payload != "xlctrlmove:") payload += "|";
        payload += n;
    }
...
wxWindow* ControllerListPanel::CreatePropertiesPanel(wxWindow* parent) {
    ...
    _headerSizer = new wxBoxSizer(wxHORIZONTAL);
    ...
    _headerSizer->Add(leftColSizer, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    ...
    _headerSizer->Add(buttonSizer, 1, wxEXPAND | wxALL, 2);
    topSizer->Add(_headerSizer, 0, wxEXPAND | wxALL, 2);
