...
LayoutPanel::LayoutPanel(wxWindow* parent, xLightsFrame *xl, wxPanel* sequencer) : xlights(xl), main_sequencer(sequencer)
{
    ...
    TreeListViewModels = CreateTreeListCtrl(wxTL_MULTIPLE, new_panel, ID_TREELISTVIEW_MODELS, "LayoutModelListCols", _("Models"), modelsTreeCols);
    ...
    TreeListViewGroups = CreateTreeListCtrl(wxTL_MULTIPLE, groups_panel_holder, ID_TREELISTVIEW_GROUPS, "LayoutGroupListCols", _("Groups"), groupsTreeCols);
    ...
    if( !is_3d ) {
        int objPage = FindNotebookPage("3D Objects");
...
wxTreeListCtrl* LayoutPanel::CreateTreeListCtrl(long style, wxPanel* panel, long id, const std::string& colOrderKey, const wxString& nameColTitle, TreeChanColumns& cols)
{
...
void LayoutPanel::showBackgroundProperties()
{
    ...
    wxString page = (Notebook_Objects != nullptr) ? Notebook_Objects->GetPageText(Notebook_Objects->GetSelection()) : wxString();
    if (page == "Controllers") {
        return;
    }
...
void LayoutPanel::SelectModelInTree(Model* modelToSelect, bool preserveFilter) {
    if (modelToSelect != nullptr) {
        ...
        const wxString targetPageName = (modelToSelect->GetDisplayAs() == DisplayAsType::ModelGroup) ? "Groups" : "Models";
        const int targetPage = FindNotebookPage(targetPageName);
    ...
    for (const auto& it : buttons) {
        if (event.GetId() == it->GetId()) {
            if (it->GetModelType() == "Add Object") {
                ...
                int modelsPage = FindNotebookPage("Models");
...
void LayoutPanel::UpdateSettingsPaneForPage() {
    ...
    wxString page = Notebook_Objects->GetPageText(Notebook_Objects->GetSelection());
    if (page == "Controllers") {
...
void LayoutPanel::ShowPropGrid(bool show) {
    if (Notebook_Objects->GetPageText(Notebook_Objects->GetSelection()) == "Controllers") {
        return;
    }
    wxAuiPaneInfo& ms = layout_mgr->GetPane("ModelSettings");
    if (show) {
...
int LayoutPanel::FindNotebookPage(const wxString& label) const {
    for (size_t i = 0; i < Notebook_Objects->GetPageCount(); i++) {
        if (Notebook_Objects->GetPageText(i) == label) return (int)i;
    }
    return -1;
}

bool LayoutPanel::IsControllersPageActive() const {
    return Notebook_Objects->GetPageText(Notebook_Objects->GetSelection()) == "Controllers";
}

wxTreeListCtrl* LayoutPanel::ActiveModelTree() const {
    if (TreeListViewGroups != nullptr &&
        Notebook_Objects->GetPageText(Notebook_Objects->GetSelection()) == "Groups") {
...
void LayoutPanel::OnNotebook_ObjectsPageChanged(wxNotebookEvent& event)
{
    ...
    wxString page = Notebook_Objects->GetPageText(Notebook_Objects->GetSelection());
    editing_models = (page == "Models" || page == "Groups" || page == "Controllers");
    if (page == "Models" || page == "Groups") {
        HandleSelectionChanged();
    } else {
#ifdef __WXOSX__
        UnSelectAllModels();
