...
#include "effects/ColorWashEffect.h"
...
ColorWashPanel::ColorWashPanel(wxWindow* parent) : xlEffectPanel()
{
	...
	BitmapButton_ColorWash_CyclesVC = new BulkEditValueCurveButton(this, ID_VALUECURVE_ColorWash_Cycles, GetValueCurveNotSelectedBitmap(), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW|wxBORDER_NONE, wxDefaultValidator, _T("ID_VALUECURVE_ColorWash_Cycles"));
	...
	BitmapButton_ColorWashCount = new xlLockButton(this, ID_BITMAPBUTTON_SLIDER_ColorWash_Cycles, wxNullBitmap, wxDefaultPosition, wxSize(14,14), wxBU_AUTODRAW|wxBORDER_NONE, wxDefaultValidator, _T("ID_BITMAPBUTTON_SLIDER_ColorWash_Cycles"));
	...
	BitmapButton_ColorWashVFade = new xlLockButton(this, ID_BITMAPBUTTON_CHECKBOX_ColorWash_VFade, wxNullBitmap, wxDefaultPosition, wxSize(14,14), wxBU_AUTODRAW|wxBORDER_NONE, wxDefaultValidator, _T("ID_BITMAPBUTTON_CHECKBOX_ColorWash_VFade"));
...
void ColorWashPanel::SetDefaultParameters()
{
    CyclesTextCtrl->SetValue("1.0");
    SetCheckBoxValue(HFadeCheckBox, false);
    SetCheckBoxValue(VFadeCheckBox, false);
    SetCheckBoxValue(ReverseFadesCheckBox, false);
    SetCheckBoxValue(ShimmerCheckBox, false);
    SetCheckBoxValue(CircularPaletteCheckBox, false);
    BitmapButton_ColorWash_CyclesVC->SetActive(false);
}

wxString ColorWashPanel::GetEffectString() {
    std::stringstream ret;

    // You must always include value curves or they wont update correctly
    if (BitmapButton_ColorWash_CyclesVC->GetValue()->IsActive()) {
        ret << "E_VALUECURVE_ColorWash_Cycles=";
        ret << BitmapButton_ColorWash_CyclesVC->GetValue()->Serialise();
        ret << ",";
    }

    if (10 != SliderCycles->GetValue()) {
        ret << "E_TEXTCTRL_ColorWash_Cycles=";
        ret << CyclesTextCtrl->GetValue();
        ret << ",";
    }
    if (VFadeCheckBox->GetValue()) {
        ret << "E_CHECKBOX_ColorWash_VFade=1,";
    }
    if (HFadeCheckBox->GetValue()) {
        ret << "E_CHECKBOX_ColorWash_HFade=1,";
    }
    if (ReverseFadesCheckBox->GetValue()) {
        ret << "E_CHECKBOX_ColorWash_ReverseFades=1,";
    }
    if (ShimmerCheckBox->GetValue()) {
        ret << "E_CHECKBOX_ColorWash_Shimmer=1,";
    }
    if (CircularPaletteCheckBox->GetValue()) {
