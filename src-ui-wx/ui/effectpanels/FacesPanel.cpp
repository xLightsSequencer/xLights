...
FacesPanel::FacesPanel(wxWindow* parent) : xlEffectPanel()
{
...
wxString FacesPanel::GetEffectString() {
    std::stringstream ret;

    if (CheckBox_Faces_Outline->GetValue()) {
        ret << "E_CHECKBOX_Faces_Outline=1,";

        if (Choice1->GetStringSelection() != "") {
            ret << "E_CHOICE_Faces_UseState=";
            ret << Choice1->GetStringSelection();
            ret << ",";
        }
    }

    if (CheckBox_SuppressShimmer->GetValue()) {
        ret << "E_CHECKBOX_Faces_SuppressShimmer=1,";
    }

    if (CheckBox_SuppressWhenNotSinging->GetValue() && !RadioButton1->GetValue()) {
        ret << "E_CHECKBOX_Faces_SuppressWhenNotSinging=1,";
        if (CheckBox_Fade->GetValue()) {
            ret << "E_CHECKBOX_Faces_Fade=1,";
        }
        if (SpinCtrl_LeadFrames->GetValue() > 0) {
            ret << "E_SPINCTRL_Faces_LeadFrames=";
            ret << SpinCtrl_LeadFrames->GetValue();
            ret << ",";
        }
    }

    if (CheckBox_TransparentBlack->GetValue()) {
        ret << "E_CHECKBOX_Faces_TransparentBlack=1,";
        ret << "E_TEXTCTRL_Faces_TransparentBlack=";
        ret << TextCtrl_Faces_TransparentBlack->GetValue().ToStdString();
        ret << ",";
    }

    ret << "E_CHOICE_Faces_Eyes=";
    ret << Choice_Faces_Eyes->GetStringSelection().ToStdString();
    ret << ",";

    ret << "E_CHOICE_Faces_EyeBlinkFrequency=";
    ret << Choice_Faces_EyeBlinkFrequency->GetStringSelection().ToStdString();
    ret << ",";

    ret << "E_CHOICE_Faces_EyeBlinkDuration=";
    ret << Choice_Faces_EyeBlinkDuration->GetStringSelection().ToStdString();
    ret << ",";

    ret << "E_CHOICE_Faces_FaceDefinition=";
    ret << Face_FaceDefinitonChoice->GetStringSelection().ToStdString();
    ret << ",";

    if (RadioButton1->GetValue()) {
        ret << "E_CHOICE_Faces_Phoneme=";
        ret << Choice_Faces_Phoneme->GetStringSelection().ToStdString();
        ret << ",";
    } else {
        ret << "E_CHOICE_Faces_TimingTrack=";
        ret << Choice_Faces_TimingTrack->GetStringSelection().ToStdString();
        ret << ",";
    }
    return ret.str();
}
...
void FacesPanel::SetDefaultParameters() {
    SetRadioValue(RadioButton1);
    SetChoiceValue(Choice_Faces_Phoneme, "AI");
    SetChoiceValue(Choice_Faces_Eyes, "Auto");
    SetChoiceValue(Choice_Faces_EyeBlinkFrequency, "Normal");
    SetChoiceValue(Choice1, "");

    if (Face_FaceDefinitonChoice->GetCount() > 0) {
        Face_FaceDefinitonChoice->SetSelection(0);
    }

    SetCheckBoxValue(CheckBox_SuppressShimmer, false);
    SetCheckBoxValue(CheckBox_Faces_Outline, false);
    SetCheckBoxValue(CheckBox_SuppressWhenNotSinging, false);
    SetCheckBoxValue(CheckBox_Fade, false);
    SetSpinValue(SpinCtrl_LeadFrames, 0);
    SetCheckBoxValue(CheckBox_TransparentBlack, false);
    SetSliderValue(Slider_Faces_TransparentBlack, 0);
}
    ...
    if (mSequenceElements != nullptr) {
        for (size_t i = 0; i < mSequenceElements->GetElementCount(); i++) {
            ...
            if (e->GetType() == ElementType::ELEMENT_TYPE_TIMING && e->GetEffectLayerCount() == 3) {
                Choice_Faces_TimingTrack->Append(e->GetName());
            }
        }
    }
