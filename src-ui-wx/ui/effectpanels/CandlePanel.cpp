...
CandlePanel::CandlePanel(wxWindow* parent) : xlEffectPanel()
{
...
void CandlePanel::SetDefaultParameters()
{
    BitmapButton_Candle_FlameAgilityVC->SetActive(false);
    BitmapButton_Candle_WindBaselineVC->SetActive(false);
    BitmapButton_Candle_WindVariabilityVC->SetActive(false);
    BitmapButton_Candle_WindCalmnessVC->SetActive(false);

    SetSliderValue(Slider_Candle_FlameAgility, 2);
    SetSliderValue(Slider_Candle_WindBaseline, 30);
    SetSliderValue(Slider_Candle_WindCalmness, 2);
    SetSliderValue(Slider_Candle_WindVariability, 5);

    SetCheckBoxValue(CheckBox_PerNode, false);
    SetCheckBoxValue(CheckBox_UsePalette, false);
}
