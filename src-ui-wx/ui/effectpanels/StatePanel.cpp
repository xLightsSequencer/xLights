...
StatePanel::StatePanel(wxWindow* parent) : xlEffectPanel()
{
    _effect = nullptr;
    _model = nullptr;
...
{
    SetEffect(nullptr, nullptr);
    SetChoiceValue(Choice_State_Mode, "Default");
    SetChoiceValue(Choice_State_Color, "Graduate");
    Choice_StateDefinitonChoice->SetSelection(0);
...
{
    auto lastTiming = Choice_State_TimingTrack->GetStringSelection();
    auto lastState = Choice_StateDefinitonChoice->GetStringSelection();
    Choice_State_TimingTrack->Clear();
    Choice_StateDefinitonChoice->Clear();

    if (mSequenceElements != nullptr) {
        for (size_t i = 0; i < mSequenceElements->GetElementCount(); i++) {
            Element* e = mSequenceElements->GetElement(i);
            if (e->GetType() == ElementType::ELEMENT_TYPE_TIMING && e->GetEffectLayerCount() <= 1) {
                Choice_State_TimingTrack->Append(e->GetName());
            }
        }
    }

    if (Choice_State_TimingTrack->GetCount() > 0) {
        Choice_State_TimingTrack->SetSelection(0);
    }
        ...
        if (m != nullptr) {
            for (const auto& it : m->GetStateInfo()) {
                if (std::find(begin(used), end(used), it.first) == end(used)) {
                    Choice_StateDefinitonChoice->Append(it.first);
    ...
    if (lastTiming != "")
        Choice_State_TimingTrack->SetStringSelection(lastTiming);
    if (lastState != "") {
        Choice_StateDefinitonChoice->SetStringSelection(lastState);
    }

    if (Choice_StateDefinitonChoice->GetSelection() == -1 && Choice_StateDefinitonChoice->GetCount() > 0) {
        Choice_StateDefinitonChoice->SetSelection(0);
    }

    SetEffect(_effect, cls);
}
