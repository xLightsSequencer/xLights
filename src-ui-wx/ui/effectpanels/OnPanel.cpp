...
OnPanel::OnPanel(wxWindow* parent) : xlEffectPanel()
{
	//(*Initialize(OnPanel)
...
void OnPanel::SetDefaultParameters()
{
    CheckBoxShimmer->SetValue(false);
    TextCtrlStart->SetValue("100");
    TextCtrlEnd->SetValue("100");
    TextCtrlCycles->SetValue("1.0");
    BitmapButton_On_Transparency->SetActive(false);
    SetSliderValue(Slider_On_Transparency, 0);
}

wxString OnPanel::GetEffectString() {
    std::stringstream ret;
    if (100 != SliderStart->GetValue()) {
        ret << "E_TEXTCTRL_Eff_On_Start=";
        ret << TextCtrlStart->GetValue().ToStdString();
        ret << ",";
    }
    if (100 != SliderEnd->GetValue()) {
        ret << "E_TEXTCTRL_Eff_On_End=";
        ret << TextCtrlEnd->GetValue().ToStdString();
        ret << ",";
    }
    if (10 != SliderCycles->GetValue()) {
        ret << "E_TEXTCTRL_On_Cycles=";
        ret << TextCtrlCycles->GetValue().ToStdString();
        ret << ",";
    }
    if (CheckBoxShimmer->GetValue()) {
        ret << "E_CHECKBOX_On_Shimmer=1,";
    }
    if (BitmapButton_On_Transparency->GetValue()->IsActive()) {
        ret << "E_VALUECURVE_On_Transparency=";
        ret << BitmapButton_On_Transparency->GetValue()->Serialise();
        ret << ",";
    } else if (Slider_On_Transparency->GetValue() > 0) {
        ret << "E_TEXTCTRL_On_Transparency=";
        ret << TextCtrlOnTransparency->GetValue();
        ret << ",";
    }
    return ret.str();
}
