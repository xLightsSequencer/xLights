...
GuitarPanel::GuitarPanel(wxWindow* parent) : xlEffectPanel()
{
...
void GuitarPanel::SetPanelTimingTracks()
{
    ...
    SequenceElements& se = frame->GetSequenceElements();
...
void GuitarPanel::SetPanelStatus(Model* cls)
{
    SetPanelTimingTracks();
}

void GuitarPanel::SetDefaultParameters()
{
    SetChoiceValue(Choice_Guitar_Type, "Guitar");
    SetChoiceValue(Choice_StringAppearance, "On");
    SetSliderValue(Slider_MaxFrets, 19);
    SetCheckBoxValue(CheckBox_Collapse, false);
    SetCheckBoxValue(CheckBox_Fade, false);
    SetCheckBoxValue(CheckBox_ShowStrings, false);
    SetCheckBoxValue(CheckBox_VaryWaveLengthOnFret, true);
    SetSliderValue(Slider_StringWaveFactor, 0);
    SetSliderValue(Slider_BaseWaveFactor, 10);
    SetPanelTimingTracks();
}
