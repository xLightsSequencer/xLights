...
const long LinesPanel::IDD_TEXTCTRL_Lines_Thickness = wxNewId();
...
const long LinesPanel::IDD_TEXTCTRL_Lines_Speed = wxNewId();
...
LinesPanel::LinesPanel(wxWindow* parent) : xlEffectPanel()
{
...
void LinesPanel::SetDefaultParameters()
{
    BitmapButton_Lines_Speed->SetActive(false);
    BitmapButton_Lines_Thickness->SetActive(false);

    SetSliderValue(Slider_Lines_Objects, 2);
    SetSliderValue(Slider_Lines_Segments, 3);
    SetSliderValue(Slider_Lines_Speed, 1);
    SetSliderValue(Slider_Lines_Trails, 0);
    SetSliderValue(Slider_Lines_Thickness, 1);
    SetCheckBoxValue(CheckBox_FadeTrails, true);
    ValidateWindow();
}
