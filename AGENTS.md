...
| `std::isnan(x)` / `std::isinf(x)` / `std::isfinite(x)` as defensive guards | Folded to `false`/`false`/`true` — guard becomes a no-op | Use `xl::isnan` / `xl::isinf` / `xl::isfinite` from `src-core/utils/FloatChecks.h`. Do **not** call `__builtin_isnan`/`__builtin_isfinite` directly: MSVC lacks them, *and* clang folds them to constants under `-ffast-math` exactly like the `std::` forms — so does the naive bit-pattern rewrite. `FloatChecks.h` launders the bits through an empty `__asm__` so the optimizer has no value fact left to fold against; that barrier is the whole point of the header, don't "simplify" it away. |
