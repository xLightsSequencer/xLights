...
---------------------------------
   -- enh (gil)   Add new model type MultiPointModel.  This is similar to PolyLine except you only get one point per node.
                  Currently there is no button to draw one of these.  Not sure if it would be practical but that can be added
                  later.  The idea is this model can be used to represent large 3D models such as a Twinkly model and every
                  node can be manipulated within the GUI.  I also anticipate adding a tool to autogenerate this model type
                  for example a model for a Firework Star could be generated by calculating all the model points.  Also any
                  model would be able to be converted to this model type and then use the GUI for position manipulation.
