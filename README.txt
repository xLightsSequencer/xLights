...
2026.06  May ??, 2026
    ...
    -change (dkulp)             Effect panels are now built from JSON metadata files in resources/effectmetadata/
                                    instead of wxSmith-generated C++ code. 47 of 55 effect panels migrated
                                    (State, Faces, Servo, and Sketch panels migrated; the State/Timing Track
                                    radio buttons on State are removed — pick one side by setting it, leave
                                    the other empty).
