...
namespace {
    template<class T>
    T CLAMP(const T& lo, const T& val, const T& hi) {
    ...
    struct ColorBuffer {
        ColorBuffer(const xlColorVector& i_cv, int i_w, int i_h) :
            cv(i_cv), w(i_w), h(i_h) {
        }
        ColorBuffer(xlColor* i_cv, int count, int i_w, int i_h) :
            cv(i_cv, &i_cv[count]), w(i_w), h(i_h) {
        }
    ...
    struct Vec2D {
        ...
        Vec2D Min(const Vec2D& p) const {
            return Vec2D(std::min(x, p.x), std::min(y, p.y));
        }
...
void PixelBufferClass::HandleLayerTransitions(int EffectPeriod, int ii) {
    ...
    if (layers[ii]->fadeInSteps > 0 || layers[ii]->fadeOutSteps > 0) {
        ...
        if (EffectPeriod < (effStartPer) + layers[ii]->fadeInSteps && layers[ii]->fadeInSteps != 0) {
            int curStep = EffectPeriod - effStartPer + 1;
        ...
        if (EffectPeriod > (effEndPer)-layers[ii]->fadeOutSteps && layers[ii]->fadeOutSteps != 0) {
            int curStep = EffectPeriod - (effEndPer - layers[ii]->fadeOutSteps);
