...
public:
    ...
    // Public so the Pathing editor (MovingHeadPanel) can detect, ahead of
    // render time, when a path crosses the point directly behind the fixture
    // (the pan/tilt fold's singularity - see CalculatePathPositions' "adjust
    // pan if pointed backwards" branch) without duplicating this math.
