...
#ifndef __ISPC_STRUCT_BarsData__
#define __ISPC_STRUCT_BarsData__
struct BarsData {
    uint32_t width;
    uint32_t height;
    int32_t direction;
    ...
    float colorsS[8];
    float colorsV[8];
    struct uint8_t4  highlightColor;
};
#endif
...
extern "C" {
#endif // __cplusplus
    extern void BarsEffectISPC(const struct BarsData * data, int32_t startIdx, int32_t endIdx, uint8_t4   * result);
#if defined(__cplusplus) && (! defined(__ISPC_NO_EXTERN_C) || !__ISPC_NO_EXTERN_C )
} /* end extern C */
#endif // __cplusplus
