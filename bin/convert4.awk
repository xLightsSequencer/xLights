...
function xml_body(m,model,maxN,seconds_array,effect,file_tr,file_array,maxModel,maxFile,models,
		     effectarray,protectedarray,headerarray,effect1array,effect2array)
		     {
			     ...
			     for(i=1;i<=maxN;i++)
			     {
				     ...
				     if(seconds!=seconds_future)
				     {
					     ...
					     if(length($0)>500)
					     {
						     ...
						     printf "\t\t\t<Effect id=\"%d\" %s startTime=\"%-7.3f\" endTime=\"%-7.3f\">%s\n",
							       id,protectedarray[n,m],seconds, seconds_future+0.0,	header;
