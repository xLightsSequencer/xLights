...
#ifdef __WXMSW__
        std::string initFileName = "xBackup.windows.properties";
#endif
#ifdef __WXOSX__
        std::string initFileName = "xBackup.mac.properties";
        std::string resourceName = wxStandardPaths::Get().GetResourcesDir().ToStdString() + "/xBackup.mac.properties";
...
#endif
#ifdef __LINUX__
        std::string initFileName = wxStandardPaths::Get().GetInstallPrefix() + "/bin/xBackup.linux.properties";
        if (!wxFile::Exists(initFileName)) {
            initFileName = wxStandardPaths::Get().GetInstallPrefix() + "/share/xLights/xBackup.linux.properties";
        }
#endif
...
void handleCrash(void *data) {
    ...
    wxString trace = wxString::Format("xBackup version %s\n\n", GetDisplayVersionString());
    ...
    std::string filename = std::string(dir.c_str()) + "/xBackup_l4cpp.log";
#endif
#ifdef __WXOSX__
    wxFileName home;
    home.AssignHomeDir();
    dir = home.GetFullPath();
    std::string filename = std::string(dir.c_str()) + "/Library/Logs/xBackup_l4cpp.log";
#endif
#ifdef __LINUX__
    std::string filename = "/tmp/xBackup_l4cpp.log";
#endif

    if (wxFile::Exists(filename))
    {
        report->AddFile(filename, "xBackup_l4cpp.log");
    }
    else if (wxFile::Exists(wxFileName(wxGetCwd(), "xBackup_l4cpp.log").GetFullPath()))
    {
        report->AddFile(wxFileName(wxGetCwd(), "xBackup_l4cpp.log").GetFullPath(), "xBackup_l4cpp.log");
    }

    if (!wxThread::IsMain() && topFrame != nullptr) {
        topFrame->CallAfter(&xBackupFrame::CreateDebugReport, report);
        wxSleep(600000);
    }
    else {
...
int xBackupApp::OnExit()
{
    static log4cpp::Category &logger_base = log4cpp::Category::getInstance(std::string("log_base"));
    logger_base.info("xBackup exiting.");

    return 0;
}

bool xBackupApp::OnInit()
{
    ...
    static log4cpp::Category &logger_base = log4cpp::Category::getInstance(std::string("log_base"));
    logger_base.info("******* OnInit: xBackup started.");
    ...
    if ( wxsOK )
    {
    	xBackupFrame* Frame = new xBackupFrame(nullptr);
    	Frame->Show();
    	SetTopWindow(Frame);
    }
    //*)
...
void xBackupApp::OnFatalException() {
    handleCrash(nullptr);
}
