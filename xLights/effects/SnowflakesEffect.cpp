...
int static possible_downward_moves(RenderBuffer &buffer, int x, int y, int z)
{
    int moves = 0;
    ...
    if (buffer.GetTempPixel(x-1 < 0 ? x-1+buffer.BufferWi : x-1, y-1, 0) == xlBLACK) {
        moves += 1;
    }
    if (buffer.GetTempPixel(x, y-1, 0) == xlBLACK) {
        moves += 2;
    }
    if (buffer.GetTempPixel(x+1 >= buffer.BufferWi ? x+1-buffer.BufferWi : x+1, y-1, 0) == xlBLACK) {
        moves += 4;
    }
...
static void set_pixel_if_not_color(RenderBuffer &buffer, int x, int y, int z, xlColor toColor, xlColor notColor, bool wrapx, bool wrapy)
{
    int adjx = x;
    int adjy = y;
    ...
    if (buffer.GetTempPixel(adjx, adjy, 0).GetRGB() != notColor.GetRGB()) {
        buffer.SetPixel(adjx, adjy, z, toColor);
    }
}
...
void SnowflakesEffect::MoveFlakes(RenderBuffer& buffer, int snowflakeType, const std::string& falling, int count, const xlColor& color1, int& effectState)
{
    ...
    for (int x = 0; x < buffer.BufferWi; x++) {
        for (int y = starty; y < buffer.BufferHt; y++) {
                ...
                xlColor color3;
                    buffer.GetTempPixel(x, y, 0, color3);
                    if (color3 != xlBLACK) {
                        // check where we can move to?
                        int moves = possible_downward_moves(buffer, x, y, ALL_Z);
                        ...
                        if (moves > 0 || (falling == "Falling" && y == 0)) {
                            ...
                            buffer.SetTempPixel(x, y, 0, xlBLACK);
                            if (y0 >= 0) {
                                // move the flake down
                                buffer.SetTempPixel(x0, y0, 0, color3);

                                if (falling == "Falling & Accumulating") {
                                    int nextmoves = possible_downward_moves(buffer, x0, y0, ALL_Z);
                                    if (nextmoves == 0) {
    ...
    while (effectState < count && check < 20) {
        ...
        int x = rand() % buffer.BufferWi;
            if (buffer.GetTempPixel(x, buffer.BufferHt - 1, 0) == xlBLACK) {
                effectState++;
                buffer.SetTempPixel(x, buffer.BufferHt - 1, 0, color1, snowflakeType == 0 ? rand() % 9 : snowflakeType - 1);

                int nextmoves = possible_downward_moves(buffer, x, buffer.BufferHt - 1, 0);
                if (nextmoves == 0) {
    ...
    if (buffer.needToInit ||
        ...
        for (int n = 0; n < Count; n++) {
            ...
            for (int check = 0; check < 20; check++)
            {
                ...
                y = y0 + (rand() % delta_y);
                if (buffer.GetTempPixel(x, y, 0) == xlBLACK) {
                    effectState++;
                    break;
                }
            }
            ...
            case 0:
                    // single node
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 0);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                    }
                break;
            case 1:
                // 5 nodes
                ...
                if (y > buffer.BufferHt - 2) y -= 1;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 1);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x - 1, y, 0, c2);
                        buffer.SetTempPixel(x + 1, y, 0, c2);
                        buffer.SetTempPixel(x, y - 1, 0, c2);
                        buffer.SetTempPixel(x, y + 1, 0, c2);
                    }
                break;
            case 2:
                ...
                if (y > buffer.BufferHt - 2) y -= 1;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 2);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
            ...
            case 3:
                ...
                if (y > buffer.BufferHt - 3) y -= 2;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 3);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        for (uint8_t i = 1; i <= 2; i++) {
                            buffer.SetTempPixel(x - i, y, 0, c2);
                            buffer.SetTempPixel(x + i, y, 0, c2);
                            buffer.SetTempPixel(x, y - i, 0, c2);
                            buffer.SetTempPixel(x, y + i, 0, c2);
                        }
                    }
                break;
            case 4:
                ...
                if (y > buffer.BufferHt - 3) y -= 2;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 4);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x - 1, y, 0, c2);
                        buffer.SetTempPixel(x + 1, y, 0, c2);
                        ...
                        buffer.SetTempPixel(x - 1, y + 2, 0, c2);
                        buffer.SetTempPixel(x + 1, y + 2, 0, c2);
                        buffer.SetTempPixel(x - 1, y - 2, 0, c2);
                        buffer.SetTempPixel(x + 1, y - 2, 0, c2);
                        buffer.SetTempPixel(x + 2, y - 1, 0, c2);
                        buffer.SetTempPixel(x + 2, y + 1, 0, c2);
                        buffer.SetTempPixel(x - 2, y - 1, 0, c2);
                        buffer.SetTempPixel(x - 2, y + 1, 0, c2);
                    }
            ...
            case 5:
                ...
                if (y > buffer.BufferHt - 2) y -= 1;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 5);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x + 1, y, 0, c1);
                        buffer.SetTempPixel(x + 1, y + 1, 0, c1);
                        buffer.SetTempPixel(x, y + 1, 0, c1);
                    }
                break;
            case 6:
                if (x < 1) x += 1;
                ...
                if (y > buffer.BufferHt - 2) y -= 1;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 6);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x + 1, y, 0, c1);
                        buffer.SetTempPixel(x, y + 1, 0, c1);
            ...
            case 7:
                ...
                if (y > buffer.BufferHt - 3) y -= 2;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 7);
                    } else {
                        buffer.SetTempPixel(x, y + 2, 0, c1);

                        buffer.SetTempPixel(x - 1, y + 1, 0, c1);
                        buffer.SetTempPixel(x, y + 1, 0, c1);
                        buffer.SetTempPixel(x + 1, y + 1, 0, c1);

                        buffer.SetTempPixel(x - 2, y, 0, c1);
                        buffer.SetTempPixel(x - 1, y, 0, c1);
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x + 1, y, 0, c1);
                        buffer.SetTempPixel(x + 2, y, 0, c1);
            ...
            case 8:
                ...
                if (y > buffer.BufferHt - 2) y -= 1;
                    if (falling != "Driving") {
                        buffer.SetTempPixel(x, y, 0, color1, 8);
                    } else {
                        buffer.SetTempPixel(x, y, 0, c1);
                        buffer.SetTempPixel(x + 1, y + 1, 0, c1);
                        buffer.SetTempPixel(x - 1, y + 1, 0, c1);
    ...
    if (driving)
    {
        for (int x = 0; x < buffer.BufferWi; ++x) {
                ...
                for (int y = 0; y < buffer.BufferHt; y++) {
                    ...
                    xlColor color3;
                    buffer.GetTempPixel(new_x, new_y, 0, color3);
                    if (color3 == xlBLACK)
                        buffer.GetTempPixel(new_x2, new_y2, 0, color3); // strip off the alpha channel
                    if (color3 == c1) {
                        buffer.SetPixel(x, y, ALL_Z, color1);
                    } else if (color3 == c2) {
                        buffer.SetPixel(x, y, ALL_Z, color2);
                    }
                }
        }
    }
            ...
            for (int x = 0; x < buffer.BufferWi; ++x) {
                    ...
                    if (color3 != xlBLACK) {
                        ...
                        case 1:
                            // 5 nodes
                            buffer.SetPixel(x, y, ALL_Z, color1);
                            set_pixel_if_not_color(buffer, x - 1, y, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x + 1, y, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x, y - 1, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x, y + 1, ALL_Z, color2, color1, wrapx, false);
                            break;
                        case 2: {
                            // 3 nodes
                            ...
                            for (int yt = 0; yt < y - 1; yt++) {
                                if (buffer.GetTempPixel(x, yt, 0) == xlBLACK) {
                                    isAtBottom = false;
                                    break;
                                }
                            }
                        ...
                        case 3:
                            ...
                            buffer.SetPixel(x, y, ALL_Z, color1);
                            for (uint8_t i = 1; i <= 2; i++) {
                                set_pixel_if_not_color(buffer, x - i, y, ALL_Z, color2, color1, wrapx, false);
                                set_pixel_if_not_color(buffer, x + i, y, ALL_Z, color2, color1, wrapx, false);
                                set_pixel_if_not_color(buffer, x, y - i, ALL_Z, color2, color1, wrapx, false);
                                set_pixel_if_not_color(buffer, x, y + i, ALL_Z, color2, color1, wrapx, false);
                            }
                            break;
                        case 4:
                            ...
                            set_pixel_if_not_color(buffer, x - 1, y + 2, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x + 1, y + 2, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x - 1, y - 2, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x + 1, y - 2, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x + 2, y - 1, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x + 2, y + 1, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x - 2, y - 1, ALL_Z, color2, color1, wrapx, false);
                            set_pixel_if_not_color(buffer, x - 2, y + 1, ALL_Z, color2, color1, wrapx, false);
                            break;
                        case 5:
                            buffer.SetPixel(x, y, ALL_Z, color1);
                            buffer.SetPixel(x + 1, y, ALL_Z, color1);
                            buffer.SetPixel(x + 1, y + 1, ALL_Z, color1);
                            buffer.SetPixel(x, y + 1, ALL_Z, color1);
                            break;
                        case 6:
                            buffer.SetPixel(x, y, ALL_Z, color1);
                            buffer.SetPixel(x + 1, y, ALL_Z, color1);
                            ...
                            break;
                        case 7:
                            buffer.SetPixel(x, y + 2, ALL_Z, color1);

                            buffer.SetPixel(x - 1, y + 1, ALL_Z, color1);
                            buffer.SetPixel(x, y + 1, ALL_Z, color1);
                            buffer.SetPixel(x + 1, y + 1, ALL_Z, color1);

                            buffer.SetPixel(x - 2, y, ALL_Z, color1);
                            buffer.SetPixel(x - 1, y, ALL_Z, color1);
                            buffer.SetPixel(x, y, ALL_Z, color1);
                            buffer.SetPixel(x + 1, y, ALL_Z, color1);
                            buffer.SetPixel(x + 2, y, ALL_Z, color1);
