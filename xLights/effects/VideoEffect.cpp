...
VideoEffect::VideoEffect(int id) :
    RenderableEffect(id, "Video", video_16, video_24, video_32, video_48, video_64) {
}
...
#define VIDEOSIZETHRESHOLD 10
    ...
    int w, h, d;
    model->GetBufferSize(bufferstyle.ToStdString(), camera.ToStdString(), transform.ToStdString(), w, h, d, settings.GetInt("B_SPINCTRL_BufferStagger", 0));
    ...
    if (_videoreader != nullptr && _videoreader->GetLengthMS() > 0) {
        ...
        if (image != nullptr && frame >= 0) {
                    ...
                    for (int x = 0; x < _videoreader->GetWidth() - xoffset - xtail; x++) {
                        ...
                        if (transparentBlack) {
                            ...
                            if (level > transparentBlackLevel) {
                                buffer.SetPixel(x + startx, y + starty, ALL_Z, c);
                            }
                        } else {
                            buffer.SetPixel(x + startx, y + starty, ALL_Z, c);
                        }
            ...
            if (durationTreatment == "Normal") {
                ...
                for (int y = 0; y < buffer.BufferHt; y++) {
                    for (int x = 0; x < buffer.BufferWi; x++) {
                        buffer.SetPixel(x, y, ALL_Z, xlBLUE);
                    }
                }
            }
        }
