...
public:
    ...
    bool operator==(const StrobeClass& r) {
        return x == r.x && y == r.y && z == r.z && hsv.hue == r.hsv.hue && hsv.saturation == r.hsv.saturation && hsv.value == r.hsv.value && color == r.color && duration == r.duration;
    }
    StrobeClass(int _x, int _y, int _z, int _duration, HSVValue _hsv, xlColor _color) {
        x = _x;
        y = _y;
        z = _z;
        duration = _duration;
...
void StrobeEffect::Render(Effect* effect, const SettingsMap& SettingsMap, RenderBuffer& buffer) {
    ...
    if (buffer.needToInit) {
        ...
        for (int i = 0; i < Number_Strobes * StrobeDuration; i++) {
            ...
            strobe.push_back(StrobeClass(rand() % buffer.BufferWi,
                                         rand() % buffer.BufferHt, rand() % buffer.BufferDp, i % StrobeDuration, hsv, color));
        }
    }
    ...
    while (strobe.size() < Number_Strobes * StrobeDuration) {
        ...
        buffer.palette.GetColor(ColorIdx, color); // take first checked color as color of flash
        strobe.push_back(StrobeClass(rand() % buffer.BufferWi,
                                     rand() % buffer.BufferHt, rand() % buffer.BufferDp, StrobeDuration, hsv, color));
    }
    ...
    while (it != strobe.end()) {
        ...
        if (Strobe_Type == 2) {
            int r = rand() % 2;
            if (r == 0) {
                buffer.SetPixel(x, y - 1, z, color);
                buffer.SetPixel(x, y + 1, z, color);
            } else {
                buffer.SetPixel(x - 1, y, z, color);
                buffer.SetPixel(x + 1, y, z, color);
            }
        }
        if (Strobe_Type == 3) {
        ...
        if (Strobe_Type == 4) {
            ...
            if (r == 0) {
                ...
                buffer.SetPixel(x + 1, y - 1, z, color);
                buffer.SetPixel(x + 1, y + 1, z, color);
                buffer.SetPixel(x - 1, y - 1, z, color);
                buffer.SetPixel(x - 1, y + 1, z, color);
            }
