...
void StateEffect::Render(Effect *effect, const SettingsMap &SettingsMap, RenderBuffer &buffer) {
    if (SettingsMap.Get("CHOICE_State_FaceDefinition", "Default") == "Rendered"
        && SettingsMap.Get("CHECKBOX_State_Outline", "") == "") {
        //3.x style State effect
        RenderState(buffer, SettingsMap["CHOICE_State_Phoneme"], "Auto", true);
    } else if (SettingsMap.Get("CHOICE_State_FaceDefinition", "Default") == XLIGHTS_PGOState_FILE) {
        RenderCoroStateFromPGO(buffer,
                               SettingsMap["CHOICE_State_Phoneme"],
                               SettingsMap.Get("CHOICE_State_Eyes", "Auto"),
                               SettingsMap.GetBool("CHECKBOX_State_Outline"));
    } else {
        RenderState(buffer,
                    effect->GetParentEffectLayer()->GetParentElement()->GetSequenceElements(),
                    SettingsMap.Get("CHOICE_State_FaceDefinition", "Default"),
                    SettingsMap["CHOICE_State_Phoneme"],
                    SettingsMap["CHOICE_State_TimingTrack"],
                    SettingsMap["CHOICE_State_Eyes"],
                    SettingsMap.GetBool("CHECKBOX_State_Outline"));
    }
}


void StateEffect::RenderState(RenderBuffer &buffer, const std::string &Phoneme, const std::string &eyes, bool outline)
{
    ...
    debug(1, "State: models = %s", (CmpNoCaseconst char*)buf + 2);
...
void StateEffect::mouth(RenderBuffer &buffer, int Phoneme,int BufferHt, int BufferWi)
{
    /*
     StatePhoneme.Add("AI");     0
     StatePhoneme.Add("E");      1
     StatePhoneme.Add("FV");     2
     StatePhoneme.Add("L");      3
     StatePhoneme.Add("MBP");    4
     StatePhoneme.Add("O");      5
     StatePhoneme.Add("U");      6
     StatePhoneme.Add("WQ");     7
     StatePhoneme.Add("etc");    8
     StatePhoneme.Add("rest");   9
    ...
    switch (Phoneme)
    {
        ...
        case 7:
            ...
            StateCircle(buffer, Phoneme,xc, yc, radius,0,360);
            break;
        case 8:       // WQ, etc
...
void StateEffect::drawline1(RenderBuffer &buffer, int Phoneme, int x1,int x2,int y1,int y2)
{
    HSVValue hsv;
...
void StateEffect::drawline3(RenderBuffer &buffer, int Phoneme, int x1,int x2,int y6,int y7)
{
    HSVValue hsv;
    ...
    /*
     ...
     yc = y7+dy;
     radius = (double) dy;
     StateCircle(Phoneme,xc, yc, radius,90,270);
     xc = x1-dy;
     StateCircle(Phoneme,xc, yc, radius,270,630);
     */
...
/*
 State draw circle
 */
void StateEffect::StateCircle(RenderBuffer &buffer, int Phoneme, int xc,int yc,double radius,int start_degrees, int end_degrees)
{
    int x,y,degrees;
    HSVValue hsv;
...
void StateEffect::drawoutline(RenderBuffer &buffer, int Phoneme, bool outline, const std::string &eyes, int BufferHt,int BufferWi)
{
    HSVValue hsv;
    double radius;
    ...
    if (eyes != "(off)") {
        ...
        yc = (int)(0.5 + Ht*0.75);
        radius = Wt*0.08;
        StateCircle(buffer,Phoneme,xc, yc, radius,start_degrees,end_degrees);
...
static bool parse_model(const wxString& want_model)
{
    ...
    pgoFile.SetFullName(_(XLIGHTS_PGOState_FILE));
    ...
    {
        wxXmlNode* Presets = xLightsFrame::FindNode(pgoXml.GetRoot(), compat? wxT("coroState"): wxT("presets"), wxT("Name"), wxEmptyString, false); //kludge: backwards compatible with current settings
...
void StateEffect::RenderCoroStateFromPGO(RenderBuffer& buffer, const std::string& Phoneme, const std::string& eyes, bool face_outline)
{
    ...
    {
        RenderState(buffer, auto_phonemes[(const char*)Phoneme.c_str()], eyes, face_outline);
        return;
    }
...
class StateRenderCache : public EffectRenderCache {
public:
    StateRenderCache() : blinkEndTime(0), nextBlinkTime(0) {
    };
    virtual ~StateRenderCache() {
    };
    int blinkEndTime;
    int nextBlinkTime;
};
...
{
    StateRenderCache *cache = (StateRenderCache*)buffer.infoCache[id];
    if (cache == nullptr) {
        cache = new StateRenderCache();
    ...
    if (type == 2) {
        RenderState(buffer, phoneme, eyes, face_outline);
        return;
    }
    if (type == 3) {
        //picture
