...
void xlTimerThread::Reset(int interval, bool oneshot, const std::string& name)
{
    ...
    spdlog::debug("Timer {} reset from interval {} to interval {} {}", _name, (int)_interval, interval, oneshot ? "ONESHOT" : "");
...
void xlTimerThread::DoSleep(int millis)
{
    ...
    else
    {
        if (millis > MIN_SLEEP_BEFORE_LOG)
        {
            spdlog::debug("THREAD {}: DoSleep({}) ... {} timer timed out", wxThread::GetCurrentId(), millis, _name);
        }
    }
}
...
wxThread::ExitCode xlTimerThread::Entry()
{
    ...
    while (!_stop)
    {
        if (_suspend)
        {
            spdlog::debug("THREAD {}: Timer {} thread suspended. Interval {}", wxThread::GetCurrentId(), _name, interval);
        ...
        if (!_stop)
        {
            ...
            //if (log)
            //{
            //    LOG_DEBUG("Timer sleeping for {}ms", (std::max)(1, (int)toSleep));
            //}
            if (toSleep > 0) {
