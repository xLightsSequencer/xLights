...
{
    wxImage img(image->width, image->height, image->data);
    ...
    for (int x = 0; x < grey.GetWidth(); x++)
    {
        ...
        {
            total += grey.GetAlpha(x, y);
        }
    }

    return (float)((double)total / ((double)grey.GetWidth() * (double)grey.GetHeight()));
}
...
int GenerateCustomModelDialog::FindStartFrame(VideoReader* vr)
{
    ...
    std::map<float, int> levelmaxlen;
    std::map<float, int> levelmaxstart;
    float level = 0.1;
    for (int i = 0; i < 9; i++)
    {
        int maxrunlength = 0;
        ...
        levelmaxlen[level] = maxrunlength;
        levelmaxstart[level] = maxrunstart;
        level += 0.1;
    }

    std::map<float, bool> suitable;
    ...
    float bestlevel = (float)((int)((last - first) / 2 * 10)) / 10.0;
