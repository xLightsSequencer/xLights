...
PianoPanel::PianoPanel(wxWindow* parent) : xlEffectPanel()
{
...
void PianoPanel::SetPanelTimingTracks()
{
...
void PianoPanel::SetDefaultParameters()
{
    BitmapButton_Piano_ScaleVC->SetActive(false);

    SetChoiceValue(Choice_Piano_Type, "True Piano");
    SetSpinValue(SpinCtrl_Piano_StartMIDI, 60);
    SetSpinValue(SpinCtrl_Piano_EndMIDI, 72);
    SetCheckBoxValue(CheckBox_Piano_ShowSharps, true);
    SetCheckBoxValue(CheckBox_FadeNotes, false);
    SetSliderValue(Slider_Piano_Scale, 100);
    SetSliderValue(Slider_Piano_XOffset, 0);

    SetPanelTimingTracks();
}

void PianoPanel::SetPanelStatus(Model* cls)
{
    SetPanelTimingTracks();
}
