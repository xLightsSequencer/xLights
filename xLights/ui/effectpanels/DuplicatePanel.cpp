...
DuplicatePanel::DuplicatePanel(wxWindow* parent) : xlEffectPanel()
{
...
void DuplicatePanel::SetDefaultParameters() {
    SetChoiceValue(Choice_Model, "");
    SetCheckBoxValue(CheckBox1, false);
    SetCheckBoxValue(CheckBox2, false);
    SetCheckBoxValue(CheckBox3, false);
    SetCheckBoxValue(CheckBox_Override_Palette, false);
    SetSpinValue(SpinCtrl_Layer, 1);
}
...
void DuplicatePanel::SetPanelStatus(Model* cls)
{
    ...
    {
        if (it != cls->GetFullName()) Choice_Model->AppendString(it);
    }
}
