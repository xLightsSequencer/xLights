    ...
    xlEffectPanel()
{
    ...
    BitmapButton_Warp_X->SetLimits(WARP_X_MIN, WARP_X_MAX);
    BitmapButton_Warp_Y->SetLimits(WARP_Y_MIN, WARP_Y_MAX);
...
void WarpPanel::SetDefaultParameters()
{
    BitmapButton_Warp_X->SetActive(false);
    BitmapButton_Warp_Y->SetActive(false);

    if (Choice_Warp_Type->GetStringSelection() == "") {
        SetChoiceValue(Choice_Warp_Type, "water drops");
        SetChoiceValue(Choice_Warp_Treatment, "constant");
    }

    SetSliderValue(Slider_Warp_X, 50);

    SetSliderValue(Slider_Warp_Y, 50);

    SetSliderValue(Slider_Warp_Cycle_Count, 1);

    SetSliderValue(Slider_Warp_Speed, 20);

    SetSliderValue(Slider_Warp_Frequency, 20);
