...
VUMeterPanel::VUMeterPanel(wxWindow* parent) : xlEffectPanel()
{
...
void VUMeterPanel::SetDefaultParameters()
{
    SetSliderValue(Slider_VUMeter_Bars, 6);
    SetChoiceValue(Choice_VUMeter_Type, "Waveform");
    SetSliderValue(Slider_VUMeter_Sensitivity, 70);
    SetChoiceValue(Choice_VUMeter_Shape, "Circle");
    SetCheckBoxValue(CheckBox_VUMeter_SlowDownFalls, true);
    SetCheckBoxValue(CheckBox_LogarithmicXAxis, false);
    SetSliderValue(Slider_VUMeter_StartNote, 36);
    SetSliderValue(Slider_VUMeter_EndNote, 84);
    SetSliderValue(Slider_VUMeter_XOffset, 0);
    SetSliderValue(Slider_VUMeter_YOffset, 0);
    SetSliderValue(Slider_VUMeter_Gain, 0);
    BitmapButton_VUMeter_YOffsetVC->SetActive(false);
    BitmapButton_VUMeter_Gain->SetActive(false);
    ValidateWindow();
    SetTextValue(TextCtrl_Filter, "");
    SetCheckBoxValue(CheckBox_Regex, false);
    FilePickerCtrl_SVGFile->SetFileName(wxFileName());
}

void VUMeterPanel::SetPanelStatus(Model* cls)
{
    Choice_VUMeter_TimingTrack->Clear();
    ...
    for (int i = 0; i < (int)mSequenceElements->GetElementCount(); i++)
    {
        ...
        {
            Choice_VUMeter_TimingTrack->Append(e->GetName());
        }
    }

    // Select the first one
    if (Choice_VUMeter_TimingTrack->GetCount() > 0)
    {
        Choice_VUMeter_TimingTrack->Select(0);
    }
