...
{
    SetTextValue(TextCtrl_SketchDef, SketchEffectSketch::DefaultSketchString());

    BitmapButton_DrawPercentage->SetActive(false);
    BitmapButton_Thickness->SetActive(false);
    BitmapButton_MotionPercentage->SetActive(false);

    SetCheckBoxValue(CheckBox_MotionEnabled, false);

    SetSliderValue(Slider_SketchBackgroundOpacity, 0x30);
    SetSliderValue(Slider_DrawPercentage, DrawPercentageDef);
    SetSliderValue(Slider_Thickness, ThicknessDef);
    SetSliderValue(Slider_MotionPercentage, MotionPercentageDef);
