...
FirePanel::FirePanel(wxWindow* parent) : xlEffectPanel()
{
...
void FirePanel::SetDefaultParameters()
{
    BitmapButton_Fire_GrowthCyclesVC->SetActive(false);
    BitmapButton_Fire_HeightVC->SetActive(false);
    BitmapButton_Fire_HueShiftVC->SetActive(false);

    SetSliderValue(Slider_Fire_Height, 50);
    SetSliderValue(Slider_Fire_HueShift, 0);
    SetSliderValue(Slider_Fire_GrowthCycles, 0);

    SetCheckBoxValue(CheckBox_Fire_GrowWithMusic, false);
}
