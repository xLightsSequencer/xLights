...
{
    for (auto it = _phonemes.begin(); it != _phonemes.end(); ++it) {
        if (wxString(*it).Lower() == wxString(phoneme).Lower())
            return true;
    }
...
void ModelFaceDialog::Paint(wxPaintEvent& event)
{
    ...
    if (FaceTypeChoice->GetSelection() == SINGLE_NODE_FACE) {
        grid = SingleNodeGrid;
        if (event.GetEventObject() != Panel_SingleNode)
            return;
    } else if (FaceTypeChoice->GetSelection() == NODE_RANGE_FACE) {
        grid = NodeRangeGrid;
        if (event.GetEventObject() != Panel_NodeRanges)
            return;
    } else if (FaceTypeChoice->GetSelection() == MATRIX_FACE) {
        grid = MatrixModelsGrid;
        if (event.GetEventObject() != Panel_Matrix)
            return;
    } else {
        return;
    }
            ...
            while (ent != nullptr) {
                if (ent->IsDir()) {
                    ...
                    if (!wxDirExists(dirname)) {
                        logger_base.debug("Extracting %s:%s to %s.", (const char*)faceZip.c_str(), (const char*)ent->GetName().c_str(), (const char*)dirname.c_str());
...
{
    if (modelPreview == nullptr || !modelPreview->StartDrawing(mPointSize))
        return;
