...
#define SAFEEXEC(fn, msg)               \
    if (SUCCEEDED(hr)) {                \
        hr = fn;                        \
        if (FAILED(hr)) {               \
            spdlog::error("---------- " msg " : {:#08x}x : {}", hr, (const char*)WindowsHardwareVideoReader::DecodeMFError(hr).c_str()); \
        }                               \
    }
...
#define DYNAMICCALL(dll, function, fn, msg)                                                                                                           \
    {                                                                                                                                                 \
        ...
        if (ffn == nullptr) {                                                                                                                         \
            spdlog::error("---------- " msg " : {:#08x}x : {}", hr, (const char*)WindowsHardwareVideoReader::DecodeMFError(E_NOINTERFACE).c_str()); \
            hr = E_NOINTERFACE;                                                                                                                       \
        } else {                                                                                                                                      \
            SAFEEXEC(ffn(fn), msg);                                                                                                                   \
        }                                                                                                                                             \
...
bool WindowsHardwareVideoReader::BitmapFromSample(IMFSample* sample, AVFrame* frame)
{
    ...
    if (FAILED(hr)) {
        ...
        spdlog::error("Device removed reason {:08X} : {}", reason, (const char*)DecodeDXGIReason(reason).c_str());
    }
...
#ifdef DETAILED_LOGGING
        spdlog::info("Read flags: 0x{:#08x} : {}", dwFlags, (const char *)DecodeReadFlags(dwFlags).c_str());
#endif
