...
Model *ModelManager::CreateModel(wxXmlNode *node, int previewW, int previewH, bool zeroBased ) const {
    ...
    if (version < 2) {
        if (node->HasAttribute("offsetXpct")) {  // Boxed Model
            ...
            float worldPos_x = previewW * offsetXpct;
            float worldPos_y = previewH * offsetYpct;
            float scalex = previewScaleX / previewW;
            float scaley = previewScaleY / previewH;
        ...
        else if (node->HasAttribute("X1")) {  // Two Point model
            float x1 = wxAtof(node->GetAttribute("X1", "0"));
            float y1 = wxAtof(node->GetAttribute("Y1", "0"));
            float x2 = wxAtof(node->GetAttribute("X2", "0"));
            float y2 = wxAtof(node->GetAttribute("Y2", "0"));
            float worldPos_x = previewW * x1;
            float worldPos_y = previewH * y1;
            float new_x2 = previewW * x2 - worldPos_x;
            float new_y2 = previewH * y2 - worldPos_y;
            ...
            node->AddAttribute("WorldPosX", wxString::Format("%6.4f", worldPos_x));
            node->AddAttribute("WorldPosY", wxString::Format("%6.4f", worldPos_y));
            node->AddAttribute("X2", wxString::Format("%6.4f", new_x2));
            node->AddAttribute("Y2", wxString::Format("%6.4f", new_y2));
        }
        else if (node->HasAttribute("NumPoints")) {  // PolyPoint model
