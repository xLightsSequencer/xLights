...
bool ESPixelStick::SetInputUniverses(Controller* controller, wxWindow* parent) {
    ...
    if (!_version.empty() && _version[0] == '4') {
        ...
        if (s_origType != type) {
            changed = true;
            for (int x = 0; x < 10; x++) {
                std::string const idx = std::to_string(x);
...
bool ESPixelStick::SetOutputs(ModelManager* allmodels, OutputManager* outputManager, Controller* controller, wxWindow* parent) {
    if (!_version.empty() && _version.front() == '4') {
...
bool EspsV4Protocol::PutSetting(std::string const& Name, std::string value, std::string const& DefaultValue) {
    ...
    do // once
    {
        ...
        // value is accepted
        Response = true;
        if (value.compare(Settings[Name]) == 0)
        {
...
bool EspsV4Protocol::PutSetting(std::string const& Name, int value, int DefaultValue) {
    ...
    do // once
    {
        ...
        if (sValue.compare(Settings[Name]) == 0)
        {
            // no need to resave an existing value
            break;
        }
    ...
    do // once
    {
        ...
        for (auto const& [ElementId, Elementval] : JsonConfig.items()) {
            ...
            if (3 > ElementId.length()) {
                ...
                ProtocolName = Lower(ProtocolName);
...
bool ESPixelStick::SetOutputsV4(ModelManager* allmodels, OutputManager* outputManager, Controller* controller, wxWindow* parent)
{
    ...
    do // once
    {
        ...
        for (int currentPortId = 0; currentPortId < cud.GetMaxPixelPort(); currentPortId++)
        {
            ...
            std::string s_currentPortId = std::to_string(currentPortId);
            ...
            std::string targetProtocolName = port->GetProtocol();
            targetProtocolName = Lower(targetProtocolName);
            ...
            if (model)
            {
                ...
                std::string colorOrder = model->GetColourOrder(channelsPerPixel > 3 ? "rgbw" : "rgb");
                colorOrder = Lower(colorOrder); // probably already lower but be sure
                Protocol.PutSetting("color_order", colorOrder, "rgb");
