...
struct GalloTechOutput {
    ...
    bool reverse{ false };
    bool ref{ false };
    int nullPixels{ 0 }; //skip is an int in the JSON, but a checkbox in the WebUI. GalloTech backend looks to support multiple nulls
    uint8_t pin{ 255 };
    bool upload{ false };

    explicit GalloTechOutput(int output_) : output(output_) { }
    void Dump() const {
...
GalloTech::GalloTech(const std::string& ip, const std::string &proxy) : BaseController(ip, proxy), _vid(0) {
    ...
    if (!json.empty()) {
        ...
        if (jsonVal.HasMember("ver") && jsonVal.HasMember("arch") && jsonVal.HasMember("name")) {
            ...
            logger_base.error("Error Determining GalloTech controller Type.");
            _connected = false;
        }

        if (_connected) {
            logger_base.debug("Connected to GalloTech controller model %s.", (const char*)GetFullName().c_str());
        }
    } else {
        _connected = false;
        logger_base.error("Error connecting to GalloTech controller on %s.", (const char *)_ip.c_str());
    }
}
...
bool GalloTech::ParseOutputJSON(wxJSONValue const& jsonVal, int maxPort, ControllerCaps* caps) {

    _pixelOutputs.clear();

    for (int i = 1; i <= maxPort; i++) {
        GalloTechOutput* output = ExtractOutputJSON(jsonVal, i, caps);
        output->Dump();
...
GalloTechOutput* GalloTech::ExtractOutputJSON(wxJSONValue const& jsonVal, int port, ControllerCaps* caps) {

    GalloTechOutput* output = new GalloTechOutput(port);
...
void GalloTech::UpdatePortData(GalloTechOutput* pd, UDControllerPort* stringData, int startNumber, bool& rgbw) const {
...
void GalloTech::UpdatePixelOutputs(bool& worked, int totalPixelCount, wxJSONValue& jsonVal) {
    ...
    //Port Pixel Count
    wxJSONValue neGalloTechS;
    for (const auto& pixelPort : _pixelOutputs) {
        if (pixelPort->upload) {
            neGalloTechS.Append(pixelPort->GetJSON());
        }
    }

    jsonVal["hw"]["led"]["ins"] = neGalloTechS;
}
...
bool GalloTech::PostJSON(wxJSONValue const& jsonVal) {
    wxString str;
    ...
    if (hnd) {
        ...
        if (ret == CURLE_OK) {
            if (buffer.find("error") != std::string::npos) {
                logger_base.error("Error From GalloTech %s", (const char*)buffer.c_str());
                return false;
            }
            return true;
        } else {
...
bool GalloTech::SetupInput(Controller* c, wxJSONValue& jsonVal, bool rgbw) {
    ControllerEthernet *controller = dynamic_cast<ControllerEthernet*>(c);
    if (controller == nullptr) {
        DisplayError(wxString::Format("%s is not a GalloTech controller.", c->GetName().c_str()));
        return false;
    }
    ...
    if (!controller->AllSameSize()) {
        DisplayError("Attempting to upload universes to the GalloTech controller that are not the same size.");
        return false;
    }
    ...
    if (o->GetType() == OUTPUT_E131 || o->GetType() == OUTPUT_ARTNET) {
        if (o->GetChannels() > 510) {
            DisplayError(wxString::Format("Attempt to upload a universe of size %d to the GalloTech controller, but only a size of 510 or smaller is supported", o->GetChannels()).ToStdString());
            return false;
        }
    }
    ...
    if (o->GetType() == OUTPUT_E131) {
        ...
        if (ddp) {
            if (ddp->IsKeepChannelNumbers()) {
                DisplayError("The DDP 'Keep Channel Numbers' option is not support with GalloTech, Please Disable");
                return false;
            }
        }
    }
...
int GalloTech::EncodeStringPortProtocol(const std::string& protocol) const {
...
int GalloTech::EncodeColorOrder(const std::string& colorOrder) const {
    ...
    if (c == "wgrb") return 36;//random guesses, GalloTech settings makes no sense
    if (c == "wbrg") return 37;
    if (c == "wrbg") return 32;
    if (c == "wbgr") return 35;
    if (c == "wgbr") return 33;
...
bool GalloTech::EncodeDirection(const std::string& direction) const {

    return direction == "Reverse";
}

GalloTechOutput* GalloTech::FindPortData(int port) {
...
const uint8_t GalloTech::GetOutputPin(int port, ControllerCaps* caps) {
...
bool GalloTech::SetOutputs(ModelManager* allmodels, OutputManager* outputManager, Controller* controller, wxWindow* parent) {

    wxProgressDialog progress("Uploading ...", "", 100, parent, wxPD_APP_MODAL | wxPD_AUTO_HIDE);
    progress.Show();

    static log4cpp::Category& logger_base = log4cpp::Category::getInstance(std::string("log_base"));
    logger_base.debug("GalloTech Outputs Upload: Uploading to %s", (const char*)_ip.c_str());

    //2105110 added json config
    //2105200 added per string null pixel to GUI but older builds have it in the JSON
    if (_vid < 2105110) {
        logger_base.error("Build 2105110 or newer of GalloTech Is Required, '%d' is Installed .", _vid);
        DisplayError("GalloTech Upload Error:\nGalloTech 0.13b5 or newer is required", parent);
        progress.Update(100, "Aborting.");
        return false;
    }

    if (_vid < 2203190 && _vid > 2112080) {
        logger_base.error("GalloTech Build 2112080 to 2203190 are broken, '%d' is Installed .", _vid);
        DisplayError("GalloTech Upload Error:\nUpload with GalloTech 0.13 and 0.13.1 is broken.\n(There is a bug in the GalloTech 0.13/0.13.1 firmware, not xLights)\nSwitch to GalloTech 0.13.2, GalloTech 0.13 beta6 or beta5 for the upload to work correctly", parent);
        progress.Update(100, "Aborting.");
        return false;
    }
    ...
    for (int port = 1; port <= maxPort; port++) {
        GalloTechOutput* pixOut = FindPortData(port);
        if(pixOut == nullptr) {
            continue;
        }
    ...
    if (!worked) {
        logger_base.error("Error Updating to GalloTech controller, JSON:%s.", (const char*)page.c_str());
    }
    ...
    logger_base.info("Uploading JSON to GalloTech.");
    progress.Update(70, "Uploading JSON to GalloTech.");
    ...
    if (!uploadWorked) {
        logger_base.error("Error Uploading to GalloTech controller, JSON:%s.", (const char*)page.c_str());
        worked = false;
    }

    logger_base.info("GalloTech Outputs Upload Done.");
