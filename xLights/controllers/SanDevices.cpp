...
std::string SanDevices::SDGetURL(const std::string& url, bool logresult) {
    ...
    if (_http.GetError() == wxPROTO_NOERR) {
        ...
        spdlog::error("Unable to connect to SanDevices {} '{}' : {} {}.", (const char*)_ip.c_str(), (const char*)url.c_str(), (int)_http.GetError(), (const char*)DecodeProtocolError(_http.GetError()).c_str());
